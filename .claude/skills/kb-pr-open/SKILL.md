...
| Area | What to check |
|------|--------------|
| **frontmatter** | All required fields present and valid: `title`, `description`, `sidebar_label`, `keywords` (8–12 items), `products` (must use the canonical product ID from `src/config/products.js` — no hyphens or underscores as word separators; `dataclassification`, `directorymanager`, `threatmanager` are correct; `data-classification`, `data_classification`, `directory-manager`, `threat-manager` are not), `tags` (must include `kb`), `knowledge_article_id` (see knowledge_article_id rules in Step 4 — its state is informational only; never propose adding, removing, or rewriting it). `sidebar_label` must not be truncated vs. `title`. **`onesecure` has a real target and should be corrected:** flag it as a Required fix and propose `1secure` (the actual `products.js` ID) — this is a normal typo/legacy-value correction, not a carve-out case. **Two values have no valid `products.js` target at all — do not flag these as Required fixes, there is no correct replacement to propose:** `general` (44 files under `docs/kb/general/` — cross-product content with no corresponding product) and `recoveryad` (1 file — genuinely ambiguous: `scripts/copy-kb-to-versions.mjs` maps both `recoveryforactivedirectory` and `identityrecovery` to the `recoveryad` KB folder, so there are two equally defensible canonical IDs and no way to pick one mechanically; ask the TSE which product the article is actually about instead of guessing). |
...
| **article-type: heading-labels** | **Resolution section heading pluralization:** Section headings in Resolution articles — both Error and Symptom formats — are always singular: `## Symptom`, `## Cause`, `## Resolution` — even when the section contains multiple items. Flag `## Symptoms`, `## Causes`, `## Resolutions` as Required fixes. |
...
| **images: location** | Check how images are referenced. KB images must be stored in a `0-images/` folder at the **product level** (`docs/kb/<product>/0-images/`), not inside category subfolders. PNG, WebP, JPG, and JPEG are all in active use and all valid — do not flag file format. Articles in category subfolders reference them with `../0-images/filename.<ext>`. Flag any `0-images/` folders created inside category subfolders rather than at the product level. |
...
| **links** | Find every internal markdown link in the article body — `[text](/docs/...)` (the dominant form, ~218 KB files) and `[text](pathname:///docs/...)` patterns to other KB articles and to versioned product docs, **and** relative `.md` links (`[text](../other-article.md)`, `[text](./other-article.md)`) — `kb_style_guide.md`'s own "Do" example for internal links uses the relative-`.md` form, and it's in active use (roughly a dozen KB files). All three forms are valid; do not propose converting one to another. For each, resolve the actual target file on disk: check for a `slug` frontmatter override on the target first; if none, the URL segment (or relative path) must match the target's real path/filename (not its `sidebar_label` or `title`). `pathname://` links specifically must use underscore version segments (`8_2`, not `8.2`) and resolve to a real file, per rulebook §8 — flag a dotted version segment as a Required fix even if the rest of the path resolves. This is a constraint on `pathname://` links where they're used, not a requirement to use that form over the bare `/docs/...` form. Flag any link whose URL does not resolve to a real file on disk as a Required fix, and correct it to the real path. Only genuinely external links (`http://`, `https://`, `mailto:`, etc.) are out of scope for this check. Backstop is `npm run build` — but `onBrokenLinks`/`onBrokenMarkdownLinks`/`onBrokenAnchors` in `docusaurus.config.js` only throw on a full build; a single-product build (`DOCS_PRODUCT=...`, the faster path `CLAUDE.md` recommends) relaxes all three to `warn`, so a broken link there won't fail the build. Either way, this check catches broken links before submission rather than relying on the backstop. |
...
|---|--------------|--------------------|
| 1 | `:::note`/`:::tip`/`:::info`/`:::warning`/`:::caution`/`:::danger`/`:::important` or `> **NOTE:**` / `> **IMPORTANT:**` blockquote inside a numbered list item | The callout must be indented **4 spaces** to attach to the preceding step. Fewer than 4 spaces breaks the list at CommonMark render time — this is a build-breaker, not a style nit. |
...
- **Raw log line / error dump titles** (rulebook §12): titles containing literal log-level tokens (FATAL, ERROR, WARN), stack noise, file paths, or truncation fragments (e.g., `ConfigurationLoader FATAL Hub Location Details Have Not Been Specified in HubDetails.xml at ...`) → normalize to `<Component> Error - <core diagnostic phrase>`, keeping only the searchable message. Examples: `ConfigurationLoader Error - Hub Location Details Have Not Been Specified`; `TraceLogger Error - Address Already in Use`; `Remote Platform Discovery Error - Could Not Get Credentials`. This form is exempt from the `Error:` *prefix* convention required for Resolution (Error) titles generally (`Error: [Unique Error Code/Message]`, per `kb_style_guide.md` and `.claude/skills/derek/SKILL.md` §3) — the inline `Error` word already disambiguates the title, so don't also prepend `Error:`. **This is one of two exemptions, not the only one:** `kb_style_guide.md` also exempts any Resolution (Error) title where the error message itself already contains the word "error" (e.g. `Agents Have Become Unresponsive Error`) — that's the broader, more common case, independent of whether the title is a normalized log dump. A Resolution (Error) title only needs the `Error:` prefix when it has none of: the prefix, the word "error" anywhere in it, or the normalized log-dump form.
