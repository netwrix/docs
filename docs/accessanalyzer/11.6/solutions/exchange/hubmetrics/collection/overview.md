...
- [EX_MetricsCollection Job](/docs/accessanalyzer/11.6/solutions/exchange/hubmetrics/collection/ex_metricscollection.md)
  – Comprised of multiple queries that use the Exchange Metrics Data Collect to process and
  ...
  received, journal, NDRs, and transport messages. These queries are configured to process and
  collect the previous 7 days of Message Tracking Logs the first time this job is run, after that
  it only collects the previous day if persistence is enabled in the query.
- [EX_MetricsDetails Job](/docs/accessanalyzer/11.6/solutions/exchange/hubmetrics/collection/ex_metricsdetails.md)
  – Collects user to user traffic per day
    ...
    not configured in the query, then most likely data collection doesn't return.
