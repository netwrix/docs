...
- Pre-Installed File System Proxy Service – Install the File System Proxy Service installation
  package on the Windows proxy servers before executing the scans. This is the recommended
  method.
...
The proxy server where the service is running conducts the data collection processing and uses a
local mode-type scan to each of the target hosts. The final step in data collection is
...
processing against the eleventh host begins after processing against the first host completes.
Proxy scanning architecture supports large deployments or widely dispersed environments.

A proxy server is any server that processes data collection against target hosts.

:::warning
You can't install the File System Proxy Service on the same server as Enterprise
Auditor.
:::
...
You configure the proxy communication during installation of the service on the proxy server, and
you configure certificate exchange options via the Applet Settings page of the File System Access
Auditing Data Collector Wizard. The installation wizard also adds the credential you provide for
secure communications to the Enterprise Auditor Connection Profile assigned to the File System
Solution.
...
servers (on the left) and with proxy servers (on the right). On the right side of the diagram, you configured the
scans to use the local host and two additional proxy servers to perform the
...
:::tip
Install the File System Proxy Service on the proxy server
before running File System scans in proxy mode as a service. After installation, you must configure
the FileSystemAccess (FSAA) Data Collector to use the service. See the
...
Sensitive Data Discovery Auditing scans also require you to install the Sensitive Data Add-on –
FSAA & SPAA Agentless (or x86) version of the Sensitive Data Discovery Add-On on the proxy server.
...
:::note
If you run Sensitive Data Discovery (SDD) scans, you'll need to increase the
minimum amount of RAM. Each thread requires a minimum of 2 additional GB of RAM per host. By
default, Enterprise Auditor configures SDD scans to run two concurrent threads. For example, if you
configure the job to scan 8 hosts at a time with two concurrent SDD threads, you need an extra 32 GB
of RAM (8x2x2=32).
