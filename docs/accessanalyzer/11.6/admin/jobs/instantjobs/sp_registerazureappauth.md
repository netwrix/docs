...
application (for example, `myorg.onmicrosoft.com`). Click **Save**. The job is now ready to run.

After the job successfully runs, it opens a browser window to Microsoft Entra ID. Log in as a
Global Administrator to grant administrator consent to the
...
- The password is the location of the PFX file generated by the script (in the \PrivateAssemblies
  ...
  for US Government-High, and 6 for US Government-DoD). To allow multiple unique certificates
  for different Microsoft Entra ID tenants to reside on the same Enterprise Auditor server, the
