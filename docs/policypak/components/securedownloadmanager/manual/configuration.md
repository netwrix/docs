...
| **Notification message should close in** | Number of seconds before the block notification auto-dismisses. Default: **60**. Uncheck to require the user to close the notification manually. |
...
Controls how many times the exclusion is applied for a given file:

| Value | Behavior |
|---|---|
| `always` | The exclusion is applied every time the file is executed. |
| `once` | The exclusion is applied the first time the file is executed. Subsequent execution attempts are blocked. |
| `onceOrForced` | The exclusion is applied the first time the file is executed, or when execution is forced by policy. |
...
including URL Pattern, File Extensions, Process Mode, and Item Level Targeting conditions —
