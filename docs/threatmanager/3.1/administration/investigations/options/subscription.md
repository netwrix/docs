...
**Step 1 –** From the investigation, click **Subscriptions**. The Subscription window opens.
...
name is: Subscription for `{{Investigation_Name}}`. This variable is replaced with the name of
...
is: Subscription to`{{Investigation_Name}}` at `{{Date}}`. These variables are replaced with the
...
**Step 10 –** Select the file format for the export from the dropdown: CSV, PDF, JSON, Excel
Viewing.
