...
To configure access to the HNAS audit logs on the Windows server hosting the Activity Monitor
activity agent, complete the following steps.
...
to select the users and groups to be audited and add the user or group you want to audit.

**Step 7 –** Select All for Type, and Full Control for Basic permissions.

After you configure access on both the Hitachi device and the Activity Agent server, configure and
enable monitoring with the Activity Monitor Console.
