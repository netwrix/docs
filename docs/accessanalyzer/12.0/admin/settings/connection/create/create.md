...
profile name after applying it to job groups or jobs, you must go back through all of those job
groups or jobs and reapply the Connection Profile.
:::


![Add User Credential](/images/accessanalyzer/12.0/admin/settings/connection/profile/addusercredential.webp)

**Step 3 –** Add credentials to this profile. Click Add User credential and the
...
:::note
If the entered passwords aren't the same, an error message pops up after you click OK
...
considering that a successful authentication doesn't automatically mean that particular credential
has the appropriate level of permissions for the data collection to occur.
:::


![Arrange Priority](/images/accessanalyzer/12.0/admin/settings/connection/profile/moveupdown.webp)

Use the Move Up and Move Down buttons to arrange priority within the User Credentials list.
...
Connection Profile. If checked, Access Analyzer applies the local login credentials before any of
...
**Step 7 –** After you add and order the user credentials, click Save and then OK to
...
changing the Connection Profile name results in breaking job groups or jobs assigned this profile.
...
**Step 4 –** After you edit the Connection Profile’s user credentials as desired, click Save
and then OK to confirm the changes to the Connection Profile.

Access Analyzer now uses the edited user credentials for authentication to target hosts for this
Connection Profile.
...
**Step 2 –** Select the user credential you want to delete from the User Credentials list. Click Delete.
...
**Step 1 –** Select the profile in the Connection Profile list and click Set as default.

**Step 2 –** The green checkmark moves. Click Save and then OK to confirm the changes.

Access Analyzer now uses this Connection Profile as the default Connection Profile.
