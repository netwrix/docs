...
To configure access to the HNAS audit logs on the Hitachi device, complete the following steps.
...
After access is configured on the Hitachi device, configure access to the HNAS audit logs on the
Windows server. See the
