...
Whenever you modify a value for an application setting within Netwrix PolicyPak, the setting is underlined. An underlined setting means that PolicyPak will deliver the configured value of that setting. For instance, if you check a checkbox
that by default is unchecked, the setting will then become underlined, stating that PolicyPak will now enforce that checked value. Simply uncheck the checkbox and the setting remains
