...
required: IP address, Web Application URL and database name. That /api is appended to
...
:::warning
that SSO applications only work via Autofill Add-on. If you are in server
...
add-on can be used in a similar way to bookmarks. The search field lets you search for the
