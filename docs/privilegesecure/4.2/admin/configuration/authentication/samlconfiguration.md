...
to be set. Incorrect settings generally results in a 404 error.
- Callback Address – The SAML provider requires the Privilege Secure callback path to be authorized.
Generally this appears by the provider after redirection. Enter the address and port of the
...
When the SAML provider authenticates a user login, it optionally signs that authentication using
...
SpPAM uses the access token to retrieve user data from the SAML provider. The SAML provider
...
user table. The exact fields to map varies between providers and according to the user’s AD
configuration.
