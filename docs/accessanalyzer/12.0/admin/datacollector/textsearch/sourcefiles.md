...
- Registry Lookup – Select this option to programmatically obtain a file path from a registry key
  ...
  Registry Browser and connect to a host to select a registry key and path to use for the
  lookup.
...
- First line is header captions line – Active only when you select CSV, TSV, or Space Separated Text
...
- Sample from host (path) – If the file you want doesn't exist on the local Access Analyzer Console,
