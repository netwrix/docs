...
available when you have a Netwrix PolicyPak client-side extension
(CSE) which supports the feature. Inside the AppSet, we've noted when a feature requires a specific
CSE version.

:::note
The PolicyPak "About Config" AppSets are still valid and available and
should be used for settings within Firefox's About:config.
:::


Additionally, if you were using any of the other main Firefox AppSets created by PolicyPak, those are now deprecated and are no longer to be used. These AppSets include:
...
![about_this_document_and_the](/images/policypak/applicationsettings/preconfigured/firefox/about_this_document_and_the.webp)

The figure shown. The PolicyPak Mozilla Firefox Pak.
