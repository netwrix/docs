...
server. See the [Password Reset Client](/docs/passwordreset/3.3/administration/password_reset_client.md) topic for configuration steps.
...
installation process. The following pages cover how to set up SSL certificates with IIS.
...
configuration should start with https://. Web browsers can be redirected to the secure URL. See
[Configuring the PRC](password_reset_client.md#configuring-the-prc) for steps to configure the start address and restricted path.
:::
...
read and write permissions to the database files. See
[Moving to SQL Server](/docs/passwordreset/3.3/administration/workingwiththedatabase/moving_to_sql_server.md) for steps to move the database and configure permissions.
...
When you delegate permissions for the Password Reset service account, Windows initially applies the
delegated permissions to all users in the domain or OU. After some time, Windows restores the original
...
If you are using an Password Reset service account with delegated permissions and don't want these
...
:::note
Windows doesn't apply changes to the AdminSDHolder container to accounts immediately. You may
