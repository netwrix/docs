...
- **Raw log line or error dump as title.** Titles like `ConfigurationLoader FATAL Hub Location Details Have Not Been Specified in HubDetails.xml at ...` — literal log lines with level tokens (FATAL, ERROR), stack noise, file paths, or truncation fragments — are unreadable and unsearchable. Normalize to the pattern `<Component> Error - <core diagnostic phrase>`, keeping only the searchable message.
  ...
  - This is a distinct rule from the `Error:` *prefix* convention (still required for Resolution (Error) titles — see `kb_style_guide.md` and `derek/SKILL.md` §3). That convention governs Resolution (Error) titles generally; this rule is specifically about a title that's a raw log dump, not a well-formed error title with a prefix. The pattern `<Component> Error - <phrase>` uses an inline `Error` word for disambiguation, not the `Error:` prefix — the two forms aren't interchangeable, but neither is "retired."
