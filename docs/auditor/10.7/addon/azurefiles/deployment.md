...
**Step 1 –** ensure you have a storage account to store logs. To reduce the volume of the stored
...
**Step 2 –** Enable audit in the Azure Files settings. Go to the Diagnostic settings, and ensure
that the following options are selected:

- "Audit" under the **Logs**
- "Archive to a storage account" under the **Destination details**
- Correct Storage account in the dropdown menu
...
source, Graph API will be used to resolve the user names. Register an Azure App
...
**CAUTION:** Don't modify the endpoint part (_/netwrix/api_).
