...
In the Quickstart example, we delivered a file to users, but PolicyPak can also
deliver scripts on the Computer side.

There are two options when you create a script policy from the Computer side.

![getting_to_know_policypak_34](/images/policypak/remoteworkdelivery/getting_to_know_endpointpolicymanager_34.webp)
...
- All policies with **Always run** selected will reapply when the PolicyPak service
  starts up.
