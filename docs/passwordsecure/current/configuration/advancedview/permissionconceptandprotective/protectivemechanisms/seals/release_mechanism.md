...
password. Even if the configuration is usually done at the level of the role, each user is responsible for his own request when carrying out the release. Even if a seal is defined for a role,
...
A modal window opens where you request the seal. The reason for the entry will be
...
[Notifications](/docs/passwordsecure/current/configuration/advancedview/clientmodule/notifications.md), as well as in the Seal
overview.

## 2. Granting a release

The [Seal overview](/docs/passwordsecure/current/configuration/advancedview/permissionconceptandprotective/protectivemechanisms/seals/seal_overview.md) can be opened via the seal symbol in the
