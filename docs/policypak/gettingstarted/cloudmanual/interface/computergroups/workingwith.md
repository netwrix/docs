...
GPMC. PolicyPak Cloud has similar concepts to Group Policy and the GPMC. Almost all
principles in the GPMC transfer here as well. The table below describes the subtle differences
between Group Policy and PolicyPak Cloud.
...
Unlike Microsoft Group Policy, a computer may be a member of multiple groups in PolicyPak Cloud. Click on the company group, then select **Add/Remove Computers from Group** and click
...
| ------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| All                 | Yes                                 | All computers are members of the **All**group. You might want to use the **All** group if you want to guarantee every computer that joins PolicyPak Cloud embraces some specific XML data files. All XML policies which are linked to the **All** group are always enforced. (This is explained a little later.) |
...
The PolicyPak CSE and the PolicyPak Cloud client get updated from time
...
CSE. This means that while PolicyPak Cloud may get new in-cloud editors or accept
policies for new components as XML data files, your endpoints won't know what to do with these
directives until their CSE is updated. Moreover, if there is a bug fix, an enhancement to PolicyPak Cloud licensing, or some special PolicyPak Cloud feature, those changes
would only make their way into the latest PolicyPak Cloud client with an update.
...
In PolicyPak Cloud, because the concept of groups is already established, you can use
...
- Set up a group of 2–5% of your computers. When a new CSE is released, you should opt in and use
  ...
  PolicyPak support team and we'll work with you.
- If all goes well, you can roll out the latest CSE to more PolicyPak Cloud groups. It
...
Testers**. Then we are configuring it to use the latest CSE and the latest PolicyPak
...
the latest version everywhere. When you do this, no groups can deliver a CSE or PolicyPak Cloud client version that is lower than what is set in the **All** group.
...
Like Group Policy, PolicyPak Cloud has the same concepts of block inheritance and
