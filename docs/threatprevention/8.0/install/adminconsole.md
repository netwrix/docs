...
- If the installer doesn't detect .NET Framework 4.7.2 or later version, a message prompts you
  to install it.
...
When you click **Next**, the End-User License Agreement page appears.
...
**Step 5 –** When you select the Win Console option, the page displays the location where the
...
**Step 6 –** On the Enterprise Manager Location Information page, the system pre-populates the host name or IP address for
the Enterprise Manager server. Enter the ports for the Enterprise Manager service
...
Manager server DNS name, hostname, or fully qualified domain name (FQDN) instead of the IP address when installing the
following:
:::
...
The Administration Console is now installed, and the Windows Console icon appears on the desktop.
