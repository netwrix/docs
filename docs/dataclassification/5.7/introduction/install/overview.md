...
4. On the Product Settings step, specify the path to install Netwrix Data Classification to. For example,
   _D:\Data\NDC\Index._
...
6. On the SQL Database step, provide the SQL Server database connection details.
    ...
    | Authentication Method | Select either the Windows or SQL Server authentication method.                                                                      |
  ...
  If the database does not yet exist, you'll need to click Create SQL Database before moving to the next step - this will automatically create the NDC database with the given name and provided configuration.
