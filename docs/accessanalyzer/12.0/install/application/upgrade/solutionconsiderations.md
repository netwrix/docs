...
- Access Analyzer 12.0 includes the Sensitive Data Discovery features in the standard
  ...
  installed, before proceeding with the upgrade.
    ...
    note of any configured Sensitive Data Criteria before upgrading Access Analyzer. You must
    reconfigure Sensitive Data Criteria after an upgrade.
    :::


- You configure Sensitive Data Criteria selection globally, and Access Analyzer uses it by default in all solution sets.
...
- The Active Directory solution by default upgrades as **Upgrade in Place**. Don't change this
...
- For Proxy Mode as a Service – You need to update the File System Proxy Service on the proxy servers.
...
- For SharePoint Agent – You need to update the Access Analyzer SharePoint Agent on the SharePoint
...
of any configured Sensitive Data Criteria before upgrading Access Analyzer. You must reconfigure
Sensitive Data Criteria after an upgrade.
:::


If you configure Sensitive Data Criteria differently for each solution, re-configure the criteria
...
the global **Settings** > **Sensitive Data** node, which Access Analyzer then uses by default in all
...
**Step 1 –** If you use the same Sensitive Data Criteria for all solutions, configure the criteria
selection at the global Settings level, which Access Analyzer then uses by default in all solution sets.
...
**Step 2 –** Select the criteria you want. Use the **Search Criteria** text field to filter the list
using keywords or expand each category to view and select individual Sensitive Data search criteria,
then click **OK**.

By default, applicable solutions inherit the Sensitive Data Criteria configured at the global Settings level.
