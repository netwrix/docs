...
pages may be badly formatted. [Contact Netwrix support](mailto:support@netwrix.com) if you
...
code. The files for the U.S. English language are:
...
correctly. A backup lets you quickly revert to a working setup.
...
from the U.S. English files, but the format is the same for all languages.
...
You will also need to edit the HTML files if your logo isn't in SVG format, or if it has a
...
files to change all instances of a message. See the [Resource Strings](#resource-strings) topic for details on editing message text.
...
To display text for all error messages, insert your text above or below the
...
message invites the user to enroll so they can also use the reset and unlock features.
