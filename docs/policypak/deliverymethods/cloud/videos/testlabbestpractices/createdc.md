...
Netwrix PolicyPak Cloud or your MDM provider.
...
### PolicyPak (all versions): How to create a DC for editing purposes

Hi. This is Whitney with PolicyPak Software. In this video, we're going to talk about
how we set up a fake DC so that we can leverage the in-the-box Group Policy to create the directives
that you will then export as XML and then deploy using the PolicyPak Cloud service.
...
But you're going to notice that we don't have a PolicyPak node, so we can't do
PolicyPak magic yet. What we need here is go to "www.policypak.com." We're going to go
to this "Customer Login" here. In this particular case, it's going to take us to the "PolicyPak CLOUD Login" and the "CUSTOMER PORTAL & CUSTOMER SUPPORT LOGIN."
...
to that "Test" GPO we looked at a moment before. Now we have that "PolicyPak" node, so
we have all of the PolicyPak superpowers available to us now without even doing a
reboot. It took just a couple seconds to download and install that MSI, and now we have that
PolicyPak magic right here so that we can deploy real Group Policy and PolicyPak magic through the cloud to your non-domain joined machines.
...
talk about what the portal looks like, and we'll go on a complete journey of what PolicyPak Cloud is.
