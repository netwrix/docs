...
Before running an Db2 0.Collection query, you must establish a connection to the appropriate IBM
Db2 server. As long as that connection is set up first, don't make configuration
...
After the connection is established, it applies to all jobs in the 0.Collection job group. It does
...
- Update Instance Name — Updates the instance name with a port if there are multiple instances in a
  single host
- AIC Permissions Import — Imports Db2 permissions to the Access Information Center (AIC)
