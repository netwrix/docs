...
- Refresh throttle time – Specify the time interval between DNS queries that the DNS Cache service
  ...
  If the DNS Cache service is used, the records are automatically updated when the TTL expires. Use
  this option to limit the number of DNS requests the service sends to update the cache. A
