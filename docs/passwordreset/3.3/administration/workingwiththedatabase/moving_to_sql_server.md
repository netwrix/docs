...
![working_with_the_database](/images/passwordpolicyenforcer/passwordreset/administration/working_with_the_database.webp)
...
wizard even if the SQL Server Compact database is empty. The wizard deletes data in the destination database before copying it from the source database.
...
![working_with_the_database_1](/images/passwordpolicyenforcer/passwordreset/administration/working_with_the_database_1.webp)
...
Configure Password Reset to connect to SQL Server immediately after copying the data. Run the Data Copy wizard again if the cutover is delayed, to update the SQL Server database with the latest data. To configure Password Reset to connect to SQL Server:
...
Open the Data Console and set your SQL Server connection options. You need to enter a password every time you open the Data Console if **SQL Server Authentication** is selected. The Data Console
