...
- Target Host – Specify the location where the Windows Firewall rule is created
    - Perpetrator Client – The client machine used by a perpetrator for the detected threat
      (typically a workstation)
...
- Credential – Select a credential profile that contains valid Active Directory credentials.
  ...
  [Integrations Interface](/docs/threatmanager/3.1/administration/configuration/integrations/overview.md). If not specified, the
  action runs as the credentials of the Action Service.
- Users – Select the users for whom to disable remote desktop access. If not specified, the user
  who triggered the threat is used.
