...
Do NOT flag contractions, heading case, passive voice, jargon, or undefined acronyms in this section — those belong to Vale and Dale respectively, **provided Vale actually ran and reported for this file.** `NetwrixKB.HeadingCase` covers all heading levels, not just the H1. Two gaps mean nothing else checks heading case for this rule: an article with no body H1 at all (Vale's heading-scoped rule never evaluates that case), or Vale not installed/erroring (the `Vale | not run (Vale not installed)` row above) — in either case, check the H1 (or the frontmatter `title` value, if there's no H1) directly per rulebook §12.
