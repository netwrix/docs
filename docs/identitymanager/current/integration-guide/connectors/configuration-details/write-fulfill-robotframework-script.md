...
This guide shows how to write a Robot Framework script that will be used by [Robot Framework](../../../integration-guide/connectors/references-connectors/robotframework).
...
required actions: `ExecuteAdd`, `ExecuteDelete` and `ExecuteModify`. These functions are where you will write the actions to perform on the external system.
...
See the [Robot Framework User Guide](https://robotframework.org/robotframework/latest/RobotFrameworkUserGuide.html) for additional information.
...
[Password Reset Settings](../../../integration-guide/toolkit/xml-configuration/connectors/passwordresetsettings) associated to the provisioned [Resource Type Mappings](../../../integration-guide/toolkit/xml-configuration/connectors/resourcetypemappings). The easiest way to enable the `Generate Password` keyword is as follow:
...
the password defined in the [Robot Framework](../../../integration-guide/connectors/references-connectors/robotframework) attribute in `appsettings.agent.json`, as well as the IP address defined in the `Variables` section.
