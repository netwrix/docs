...
The following example sends a notification email to the email address, stored in `Mail`, of the users from `Directory_User` targeted by the workflow, so here the new user created by the `Directory_User_StartInternal` workflow.
...
|IsCC<br/><span class="optionalAttribute">default value: false</span>|<p>**Type**<br />Boolean<br /><br />**Description**<br />`true` to send the notification email to the recipients as a carbon copy (CC).</p>|
