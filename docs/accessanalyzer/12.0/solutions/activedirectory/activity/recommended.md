...
By default, the **Active Directory** > **6.Activity** Job Group runs with the default settings. You
...
- Successfully execute the **Active Directory** > **5.Domains Job Group** before running the
...
Set Connection Profiles directly on the
[0.Collection > AD_ActivityCollection Job](/docs/accessanalyzer/12.0/solutions/activedirectory/activity/ad_activitycollection.md) to connect to either
...
:::info
Run from the 6.Activity Job Group level to correlate 0.Collection job
...
Multiple Access Analyzer Consoles aren't supported. Run this group from a single Access
Analyzer Console.
...
**Step 7 –** Run from the **6.Activity** Job Group level to correlate 0.Collection job
group data with other jobs.
