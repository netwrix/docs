...
Use the Scope page to define which folders this query searches. It
...
- Default Public Folders – User can access these folders directly with client applications such as
  ...
  when you expand a public folder tree.
- System Public Folders – Users can't access these folders directly. Client applications, such as
  ...
  folders, such as the EFORMS REGISTRY folder, that don't exist in general-purpose public folder
  trees.
...
When you select Scope to **Selected Public Folders** on the Scope page, the options to specify the
desired folders become available.
...
- Select public folders from – Enter the name of the server hosting the public folders you want and
...
The wizard adds the selected public folders to the table at the bottom. Click **Remove** to delete a
selected word from the filter list.

## Scope to Selected Table

When you select Scope to **Selected Table** on the Scope page, the options to specify the desired
...
- Table Name – retrieves the list of selected public folders from a Enterprise Auditor database
  ...
  database.
    ...
    - Filter the Table name box by entering a name in the textbox and clicking **Retrieve**

- Field name – Select the table you want, and the available fields populate the Field names box.
