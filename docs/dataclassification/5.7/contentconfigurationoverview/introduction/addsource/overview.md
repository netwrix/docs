...
| Write classifications       | Select if you want to write classifications directly into the document properties, i.e. use tagging. This applies to DOC/DOCX/XLS/XLSX/PPT/PPTX/PDF. See also [Manage File System](/docs/dataclassification/5.7/contentconfigurationoverview/introduction/manage/managefilesystem.md).                                                                                                                  |
...
| Anonymous Access Allowed    | Select this option to disable security filtering for the content source. If cleared, the indexing processes will collect Windows Access Control Lists (ACLs) for the files, and search results will be filtered based on the end user's Windows identity.                                                                                                                                             |
| Duplicate Detection Enabled | Select to exclude duplicates (i.e. documents that contain the same text content) from the index.                                                                                                                                                                                                                                                                                                        |
| Re-Index Period             | Specifies how often the source should be checked for changes. Default is **7 days**. <br />**NOTE:** Netwrix Data Classification monitors file shares to detect when a document is added/modified. These will then be queued for reprocessing. The source will still be checked for changes based on the re-index period in case any updates aren't received. |
| Priority                    | Use default values.                                                                                                                                                                                                                                                                                                                                                                |
| Document Type               | Specify a value that will be used to restrict queries when using the search index.                                                                                                                                                                                                                                                                                                                  |
...
| Upload                   | If selected, the file will be uploaded into the NDC SQL database. This will allow the program to present the file to users even if they don't have access to the original file location.                                                                   |
| Text Patterns            | [See Text Processing for more information.](/docs/dataclassification/5.7/systemconfigurationoverview/configuration/texthandling.md)                                                                                                                         |
