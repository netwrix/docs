...
A connection represents a link between a [connector](/docs/identitymanager/current/integration-guide/toolkit/xml-configuration/connectors/connector) and a connection package.
...
Configure the connection settings in the `appsettings.agent.json` file by adding an `ADExportFulfillment` section to the `Connections` object, for example:
...
Details about these settings can be found in Identity Manager's [connector references](/docs/identitymanager/current/integration-guide/connectors/references-connectors).
...
The following example filters the CSV files produced by the export of the `Directory` connection, to keep only German sites, i.e. the rows where `Identifier` starts with `DE_`.
...
The following example merges the `Company`, `Subsidiary`, `Department` and `Team` columns of the `Directory_UserRecord` table, output of the export of the `Directory` connection, to concatenate the 4 properties into a single `FullOrganization` property.

Setting `RemoveEmpty` to `true` means that rather than having an organization such as `Contoso//HR/Payroll`, the result is `Contoso/HR/Payroll`.

Setting `RemoveDuplicates` to `true` means that rather than having an organization such as `Contoso/Contoso/HR/Payroll`, the result is `Contoso/HR/Payroll`.
...
| DatePattern optional | **Type:** String **Description:** Format of the transformed dates to be stored when the original object isn't a date, when the transformation type is `TransformDate`. **Note:** This property is needed when using CSV files which store everything as strings, including dates. |
