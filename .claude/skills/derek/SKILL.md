...
- Contains a Symptom heading (`## Symptom` or `## Symptoms`), a Cause heading (`## Cause` or `## Causes`), and a Resolution heading (`## Resolution` or `## Resolutions`) → **Resolution**. Flag any plural spelling as a rename per §2 — the plural form still counts as a structural match here; it just also gets a §2 finding.
  ...
  - Having classified by content, *then* check the title against §3: a Resolution (Error) title must either start with `Error:` or already be in the normalized `<Component> Error - <phrase>` log-dump form (rulebook §12) — flag a Required fix only when it is neither. This is the check that a title-based classifier can never make, because it would always find the classification and the check in agreement by construction.
