...
1. The add-on comes with a special set of alerts developed by Netwrix industry experts. With the
   help of a straightforward command-line tool, you upload these alerts to Netwrix Auditor and
   enable integration with the add-on.
2. Whenever the alert is triggered, the add-on retrieves an Activity Record for this action using
...
3. The add-on creates an incident ticket in ServiceNow, populates it with data that was available in
   the alert, and assigns it to a proper team. Now, you can process a ticket as usual.
