...
[Activity Groups Page](/docs/privilegesecure/26.03/admin/activities/activitygroups/activitygroups.md) topic for additional information.
...
[Add Action Window](/docs/privilegesecure/26.03/admin/activities/addaction/addaction.md) topic for additional information.

![Activities Page](/images/privilegesecure/26.03/accessmanagement/admin/policy/page/activitiespage.webp)
...
- Green + button — Create a new activity. See the [Add Activity](/docs/privilegesecure/26.03/admin/activities/activity/activity.md) topic for
...
[Login Account Templates](/docs/privilegesecure/26.03/admin/activities/activity/activityloginaccounttemplates.md) topic for additional information.
- Create Account checkbox — Indicates whether an account is created at the beginning of the activity
if it doesn't already exist. When the Activity starts, a check is made to determine if an account
...
- Delete After Use checkbox — Indicates whether the account is deleted at the end of the
activity. If an account was created to perform the activity, check this option to remove the
account after the activity is complete. Otherwise, the account is disabled at the end of the
...
:::note
A built-in administrator account can't be deleted or disabled at the end of a
session.
:::
...
requester rights to the activity. See the [Connection Profiles Page](/docs/privilegesecure/26.03/admin/accesspolicy/connectionprofiles/connectionprofiles.md)
...
to Active Directory.
- Connect Account — Provides a list of service accounts that is used by the Proxy Service for
impersonation for Interactive App Launch activities. If no account is selected, the account
specified by the Login Account Template is used and no impersonation will occur. This option
...
Microsoft Entra ID or Website. See the [Microsoft Entra ID Details Page](/docs/privilegesecure/26.03/admin/resources/detailspages/entraid/entraid.md) and
[Website Details Page](/docs/privilegesecure/26.03/admin/resources/detailspages/website/website.md) topics for additional information.
...
action it is paired with. See the [Add Action Window](/docs/privilegesecure/26.03/admin/activities/addaction/addaction.md) topic for
additional information.

:::note
It isn't possible to edit the Action Type. Delete the existing action and then create a
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Requester | The Requester login account type will use the user's own account to run the activity. The user either logs on to the resource directly or connects to the resource through the proxy. In both cases the user has to enter their user name and password. This login account should be used to avoid having a separately managed account. The user needs to have a matching account on the endpoint and needs to know the password to login. |
| Managed | The Managed login account type will used an account managed by Privilege Secure to run the activity. After created, a Managed account will persist to the endpoint. When a session ends or is canceled, the password is automatically rotated. The account will not be removed afterward, but it is disabled when at rest. The primary use case is for instances where the user desktop experience should persist across sessions. A Managed account can be a specific account name or based on any variable added to the Login Account Template. The password for a managed account is available to the user via the UI during an active session. |
| Activity Token | The Activity Token login account will use a unique time-limited ephemeral account created when the Activity is started and removed when it is completed. The account name is automatically generated from the user’s sAMAccountName and Session ID, filled out to as many characters as configured for the activity token. See the [Activity Token Complexity Page](/docs/privilegesecure/26.03/admin/accesspolicy/activitytokencomplex/activitytokencomplexity.md) topic for additional information. The account name can be entirely random or based on variables or text added to the Login Account Template. The primary use case is where the user profile should be destroyed after each session and have the user log-in to a clean desktop every time they connect; a common use case for remote vendors and contractors. |
