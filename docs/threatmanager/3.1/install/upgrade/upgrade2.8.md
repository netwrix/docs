...
To upgrade Threat Manager 2.6/2.7 to 2.8 or to apply a hotfix to Threat Manager:
...
- Show Advanced Settings – Advanced settings contains the following parameters:
    ...
    - Reduce Source Databaze Size – This action will lead to the migration dropping the indexes in
      the old databases, before performing the migration
        ...
        :::note
        This setting isn't recommended as it will leave the old database in a broken
        ...
        This option also reveals a **Delete Source Database** checkbox. After exporting the data
        from the old database, the old database is deleted.
        ...
        If the migration fails, manually import the data into the new database
...
detects the existing PostgreSQL database and prompts to migrate.
...
Threat Manager won't work. To eliminate downtime, install Netwrix Threat Manager 2.8 with a new database and
direct event streams to that server after the installation is complete.
:::


## Clean up Dependencies

After installation, uninstall the following dependencies associated with the previous version of
Threat Manager:
...
Clear the cache of the browser used to view the Threat Manager Console before launching Threat
Manager.
...
identified by navigating to the Integrations page in Threat Manager. If an app token is deprecated, create a new one for the application to use.

### Identify and Update App Tokens

To identify and create new app tokens:
