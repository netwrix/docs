...
- Level – The relative severity level, or risk level, of the threat. See the
  [Fine Tune a Threat](/docs/threatmanager/3.1/administration/configuration/threatdetection/threatconfiguration.md) topic for additional information.
...
- SIEM – A green check mark indicates that threat information is sent to a SIEM service when
  ...
  disabled.
- Playbook – A green check mark indicates that a Playbook is assigned to the threat. A Playbook is automatically executed every time a threat of this type is detected.
- Rollup – A green check mark indicates that rollups are enabled. A gray x indicates that rollups
  aren't enabled. See the [Fine Tune a Threat](/docs/threatmanager/3.1/administration/configuration/threatdetection/threatconfiguration.md) topic for additional
  information.
