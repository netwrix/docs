---
title: "Why is PolicyPak Preferences (original version) \"forced disabled\" by default?"
description: "Why is PolicyPak Preferences (original version) \"forced disabled\" by default?"
sidebar_position: 70
---

# Why is PolicyPak Preferences (original version) "forced disabled" by default?

Starting with build 2862, Netwrix PolicyPak Preferences are
...
PolicyPak Preferences' job is to deliver GPPreferences settings to machines with
PolicyPak Cloud, or alongside your MDM service. The typical use case is when the
...
When the machine is not domain joined, there is not a problem with PolicyPak
Preferences. PolicyPak Preferences will take any GPPreferences setting and get it
...
Policy, the PolicyPak Preferences component can cause an issue because of timing
outside of our control between the Group Policy / GPPreferences engine and PolicyPak.
This has been a known issue for years. You can
see [Why do I see slowdowns on my machines when PolicyPak Preferences is licensed and computers domain joined? Can this be worked around?](/docs/policypak/components/admintemplatesmanager/knowledgebase/exportinggrouppolicypreferences/domainjoined.md)

In order to minimize conflicts, we have, in the past, suggested that customers un-license PolicyPak Preferences when the machine is domain joined.
...
example snip from a Universal license file, you can see that PolicyPak Preferences is
...
on with the ADMX setting entitled 'Specifically enable_ PolicyPak _Preferences
...
you a license in return, with PolicyPak Preferences automatically set with the
ENABLED=FALSE flag, as expressed earlier. But there are times when license files might be sent with
this omitted, thus turning on PolicyPak Preferences and causing conflicts in a domain.
...
![655_1_image001_950x529](/images/policypak/license/unlicense/655_1_image001_950x529.webp)

Starting with build 2682, it will now take two steps for PolicyPak Preferences to be
enabled:
...
Then and only then will PolicyPak Preferences attempt to process GPPreferences items
with domain joined machines.

## How does this affect non-domain joined machines?

If your machine is non-domain joined and you wish to have PolicyPak deliver
GPPreferences using PolicyPak Preferences, you only need to have one step:

- PolicyPak Cloud + Non Domain joined machines: Do nothing. The PolicyPak Preferences license in PolicyPak cloud will enable you to deliver
  GPPreferences to non-domain joined machines.
- PolicyPak + an MDM service like Intune + Non Domain joined machines: You need to
  ensure your Universal license has the PolicyPak Preferences line enabling (or
  omitting) enablement.
...
domain joined, but you deploy GPPreferences from PolicyPak Cloud, or using PolicyPak and your MDM service like Intune.

Remember though, the problem arises when you deliver GPPreferences settings from multiple sources,
say, on-prem GPOs and also PolicyPak Cloud. Or on-prem GPOs and with PolicyPak and your MDM service.
...
- Use the on-prem GPO delivery of GPPreferences (and keep the PolicyPak Preferences
  component disabled),

or

- Use the Cloud or MDM delivery of GPPreferences, where you must enable PolicyPak
...
away from on-prem GPPreferences and use CLOUD or MDM with PolicyPak exclusively.

:::note
If PolicyPak Preferences license is disabled by ADMX policy (see below),
...
![forcedisabled1](/images/policypak/license/unlicense/forcedisabled1.webp)

In the future, we plan for PolicyPak Preferences to evolve to enable co-existence from
multiple sources.
