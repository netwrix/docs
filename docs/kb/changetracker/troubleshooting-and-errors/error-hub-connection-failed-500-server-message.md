...
> **IMPORTANT:** Use this method to resolve this problem only if the Agent you are trying to connect is the same device or a re-imaged variant of the older device. Contact [Netwrix Change Tracker Support](https://www.netwrix.com/sign_in.html?rf=tickets.html#/open-a-ticket) before attempting this fix.
...
4. Erase the ID currently in that file and paste in the ID copied from the preceding rolling log. Ensure there are no spaces at the beginning or end of the text string.
5. Delete all files within the `%ProgramData%\NNT\gen7agent.service` folder except the `HubDetails.xml` and `agent.id` files.
6. Edit the `HubDetails.xml` file and erase the `<E1>` tags and everything between them.
7. Replace the `<Password />` entry in `HubDetails.xml` with `<Password><YourAgentPassword></Password>`.
...
8. If you have specified a Prefix or Suffix and want to remove it, replace `<NamePrefix>MyPrefix</NamePrefix>` with `<NamePrefix />`.
9. Save the `HubDetails.xml` file. You may need to save it to your desktop and copy it back to the original location due to hardened permissions.
...
4. Erase the ID currently in that file and paste in the ID copied from the preceding rolling log. Ensure there are no spaces at the beginning or end of the text string.
5. Delete all files except the `HubDetails.xml` and `agent.id` files within the `/var/nnt/gen7agent.service` folder.
6. Edit the `HubDetails.xml` file and erase the `<E1>` tags and everything between them.
7. Replace the `<Password />` entry in `HubDetails.xml` with `<Password><YourAgentPassword></Password>`.
...
8. If you have specified a Prefix or Suffix and want to remove it, replace `<NamePrefix>MyPrefix</NamePrefix>` with `<NamePrefix />`.
9. Save the `HubDetails.xml` file.
...
- How to Move Your Linux Server MongoDB Database to a Windows Server (kb.netwrix.com/8283) — SME meeting flagged; Linux Hub retired in 8.x
- Add Palo Alto Device to a Proxy Agent as a Proxy Device in Change Tracker Gen 7 (kA04u0000000JXHCA2) — SME meeting flagged
-->
