...
A host list is a grouping of hosts to execute jobs against. Every host list
...
option in the Activities pane or right-click menu, either at the Host Management node with
...
:::info
Don't modify the criteria after you create a dynamic-based list. To modify a
dynamic-based list, delete and recreate it instead.
:::


## Static Host Lists

You create static host lists either through host discovery queries or by manually entering them within the
...
only be changed manually. You frequently create custom host lists to scope a job to
...
- Select multiple hosts from the data grid using the Windows Ctrl and left-click function, from the
  Host Mast Table or any host list under the Host Management node. Then use the
