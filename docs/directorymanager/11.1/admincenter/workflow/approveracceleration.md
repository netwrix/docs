...
automatically accelerates a request to another approver if the current approver doesn't act on it
for a certain number of days.

Let’s assume User A, who is the approver of a workflow, doesn't approve/deny a request for x number
of days. This request will accelerate to User B (first acceleration level), and if User B doesn't
...
It continues to appear in the list of pending requests for the approvers.
...
4. On the **Workflows** page, click the **Advanced Workflow Settings** tab.
    ...
    :::note
    When an SMTP server isn't defined for the identity store, approver acceleration settings
...
5. Use the **Approver Acceleration** toggle button to apply the approver acceleration settings and
   ...
   To exempt a particular workflow, open it and clear the **Approver Acceleration** checkbox. See
...
6. In the **Maximum Levels** box, specify a number, say 2. Workflow requests would be accelerated to
   a maximum of 2 levels for approval. Requests that aren't approved or denied at the maximum level
   become static.
7. In the **Repeat Every X Days** box, specify x number of days, say 5. When an approver doesn't
...
When the approver of a workflow doesn't approve/deny a request within a certain number of days,
...
1. When the approver is a user who doesn't approve/deny a request for 5 days, the request is
   accelerated to the user’s primary manager.  
   Example: A workflow request is with User A, who doesn't approve/deny it. After 5 days, the
   request will be accelerated to User A’s manager (first level acceleration).
2. If the new approver (User B) doesn't approve/deny the request in the next 5 days, the request is
   accelerated to User B's primary manager (User C) - second level acceleration.
3. If the new approver (User C) doesn't approve/deny the request in the next 5 days, the request is
   accelerated to User C’s primary manager (User D) – third level acceleration).
4. After the third (and the last) level acceleration, if User D doesn't approve or deny the request
   for 5 days, the request goes to the default approver. If the default approver isn't specified,
...
2.  If the hierarchy breaks at any point in the acceleration chain, the request is routed to the
    default approver. Referring to the example in step 3, if User C doesn't have a manager, the
...
hereafter don't have the authority to approve or deny the request. The request is considered
...
2. If Group A’s primary owner (Object O) doesn't approve/deny the request in the next 5 days, the
   following happens:
...
3. After the third (and the last) level acceleration, if the approvers don't approve or deny the
   request for 5 days, the request goes to the default approver. If the default approver isn't
...
1. When the default approver is a user who doesn't approve/deny a request for 5 days, the request
   becomes static.
2. When a default approver isn't set in advanced workflow settings, requests can't be accelerated
...
3. When a group is set as the default approver, workflow requests are accelerated to all group
   members.
4. If members of the default approver group don't approve/deny a request for 5 days, it is
   accelerated to the group's primary owner.
5. If the default approver group has no members or when all members are disabled, the request is
   accelerated to the group's primary owner.
6. However, if the owner of the default approver group is also a group, the workflow request isn't
   accelerated and becomes static.

### Other Cases

1. In a circular manager situation, requests are auto-approved.  
   **Example:** User A is the approver of a workflow, who doesn't approve/deny a request for 5
   days. The request is accelerated to User A’s manager – User B (first level acceleration). If User
   B doesn't approve/deny the request in the next 5 days, the request goes to User B’s manager –
   User C (second level acceleration). If User C doesn't approve/deny the request in the next 5
...
2. When the requester comes up as an approver in the approval acceleration chain, the request is
   auto approved.  
   Example: User A doesn't approve/deny a request for 5 days. The request is accelerated to User
...
3. For the _Transfer a User_ workflow, a request goes to the new primary manager of the requester
   ...
   User C doesn't approve/deny it for 5 days, the request will move up User C’’s acceleration
   chain.
4. When the approver, with whom a request resides, is removed as manager or group owner,
...
5. If an administrator re-routes a workflow request to another user (User B) from the Directory
   ...
   Note: this feature uses the new manager’s line for acceleration.
