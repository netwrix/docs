...
| Managed | The Managed login account type uses an account managed by Privilege Secure to run the activity. Once created, a Managed account persists to the endpoint. When a session ends or is canceled, the password is automatically rotated. The account isn't removed afterward, but it is disabled when at rest. The primary use case is for instances where the user desktop experience should persist across sessions. A Managed account can be a specific account name or based on any variable added to the Login Account Template. The password for a managed account is available to the user via the UI during an active session. |
