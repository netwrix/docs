...
Passwords shouldn't contain keyboard patterns because they are vulnerable to cracking attacks and
...
- Choose the pattern detection mode from the **keyboard patterns** dropdown list. The Horizontal
...
- Select the **Detect direction change** checkbox if Password Policy Enforcer should detect
...
- Select the **Detect key repeat** checkbox if Password Policy Enforcer should detect keyboard
...
- Select the **Detect key skip** checkbox if Password Policy Enforcer should detect keyboard
...
- Choose a value from the **Tolerance** dropdown list to specify the longest keyboard pattern that
  Password Policy Enforcer tolerates before rejecting a password. For example, the password
  "my**qwer**pw" contains a four-character keyboard pattern (shown in bold type). Password Policy
  Enforcer rejects this password if the tolerance is set to three or lower, and accepts it if
...
:::note
Modifier keys such as Shift and AltGr won't evade pattern detection. Key positions can
differ, even in keyboards with matching layouts. The Keyboard Pattern rule may not detect some
patterns because of these differences. Report any missed patterns to
