...
The following how-to assumes that you have already read the topic on [Indirect Permissions](../../integration-guide/role-assignment/indirectpermissions).
...
After running a [Compute Role Model Task](../../integration-guide/toolkit/xml-configuration/jobs/tasks/server/computerolemodeltask), Indirect Permissions should now appear for your test user.
...
![AD Assigned Resource Navigations](/images/identitymanager/adassignednavigations_5.2.1.webp)
...
- What is the target Entity Type?
 Once again, will configure a rule for nominative users. The Entity Type is ```MicrosoftEntraID_DirectoryObject_NominativeUser```.
...
configure a rule for nominative users. The Entity Type is `MicrosoftEntraID_DirectoryObject_NominativeUser`.
...
<IndirectResourceRule ResourceType="MicrosoftEntraID_DirectoryObject_NominativeUser" Property="memberOf" Correspondence="SharePointObject" CorrespondenceMembershipProperty="Group" Entitlement="Entitlement" />
