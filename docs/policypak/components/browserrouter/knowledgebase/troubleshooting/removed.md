...
From a Netwrix PolicyPak perspective, and specifically PolicyPak browser router, with IE going away/ gone, we have made some code changes to
compensate for it.

Once IE is removed (for real, or using the simulation in policy from the Microsoft article above...)
the follow behavior is now expected with latest CSE and PolicyPak Browser Router:
