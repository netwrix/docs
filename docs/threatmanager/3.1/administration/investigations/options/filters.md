...
data sources have been configured, there is also a Source dropdown. Filter statements can be
...
![Investigations Interface showing the Timeframe dropdown](/images/threatprevention/7.5/reportingmodule/investigations/options/timeframe.webp)
...
To configure a custom timeframe range:
...
- Event ( group header in the menu):
    ...
    - Perpetrator Type – The type of account (e.g., user, service) that initiated the event
...
You can create a filter for a new investigation or modify a filter for a saved investigation. To
build a filter statement:

**Step 1 –** Navigate to the investigation's Filters section.
...
**Step 4 –** Select an attribute from the Attribute dropdown.

**Step 5 –** Select an operator from the Operator dropdown.

**Step 6 –** Enter a value in the Filter box. Possible values available from the events will
populate in a dropdown as you type. Select the value you want from the dropdown. If the
value you type isn't available in the dropdown, use the Add button to add it to the Filter
box.
...
After the filter is set, click **Run Query** to generate the report ad hoc. This lets you test
whether your filter statement is working correctly. Save the investigation for reuse. You can also
add subscriptions or export the report data using the options above the Filters section.
