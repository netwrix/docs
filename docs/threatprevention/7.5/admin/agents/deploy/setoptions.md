...
If unchecked, specify the installation location, for example, d:\myagent.
...
Once these settings are configured, the Agent is ready for deployment on the selected
...
This information is stored in the SIWindowsAgent.exe.config file as XML tags and values in the folder where
the Agent is installed. When the Agent Service starts, it reads this file.
...
:::warning
Select the settings for the Agent on this window carefully, such as the
Enable DNS Host Name Resolution and Safe Mode options, even when they're enabled for the
