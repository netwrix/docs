...
How the synchronization has been configured in the
...
is done via https port 443, which must be enabled on the server side. After the prerequisites have
...
is changed in the App. "password" is changed in the app, you'll have different data statuses on
both devices. After a synchronization, you'll receive the changed user name and the new password
on both devices.
