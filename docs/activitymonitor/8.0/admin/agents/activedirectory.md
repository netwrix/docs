...
monitor Active Directory, deploy an AD agent to every domain controller,
including the read only domain controllers. You can deploy the agents in batches.

## Deploy the AD agents to the domain controllers in the target domain
...
:::note
The Activity Monitor will validate the Host Name or IP Address entered in the
...
**Step 10 –** Click **Finish**. The Add New Agents window closes, and the activity agent is
