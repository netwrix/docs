...
The FS_QuarantineData job is designed to quarantine files that are pending cleanup.
...
:::warning
Don't enable the actions unless they are required. Disable the actions after execution
to prevent making unintended and potentially harmful changes to Active Directory.
:::


:::warning
Don't modify the action tasks. The action tasks are preconfigured for this job.
:::
...
Before running the FS_QuarantineData job, select the **Give domain user full
