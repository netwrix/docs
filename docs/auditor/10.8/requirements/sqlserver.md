...
| **`Netwrix_CategoriesDB`**      | Intended for integration with Netwrix Data Classification. This database is always created but is involved in the workflow only if the DDC Provider is enabled. See [Sensitive Data Discovery](/docs/auditor/10.8//admin/settings/sensitivedatadiscovery.md) for more information. |
