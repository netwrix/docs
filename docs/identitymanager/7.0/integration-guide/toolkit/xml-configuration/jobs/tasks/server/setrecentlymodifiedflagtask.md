...
description: "When synchronizing in full or incremental mode, optimize the compute performance of the role model by taking into account only the changes made by the synchronization."
sidebar_position: 21
---

When synchronizing in full or incremental mode, optimize the compute performance of the role model by taking into account only the changes made by the synchronization. This optimization is based on the `dirty` property of the entity [Resource](/docs/identitymanager/current/integration-guide/toolkit/xml-configuration/resources/resource). The task [Usercube-Compute-RoleModel](./computerolemodeltask) with option `dirty` set to `true` will treat only resources marked as dirty.
