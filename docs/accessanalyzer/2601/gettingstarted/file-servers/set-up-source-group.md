...
**Step 2 –** Select **File Server** and click **Next**.

**Step 3 –** Enter a **Source Group Name**.

**Step 4 –** Select a service account from the **Service Account** dropdown, or click **+** to create one inline. See [Service Accounts](../../configurations/service-accounts/overview.md) for details.

**Step 5 –** Click **Add** under **File Servers** and enter the server name or IP address of each file server to include. Click **Done** when finished.

**Step 6 –** Click **Test Connection** to verify connectivity. Each server displays a **Connected** or **Failed** status. Resolve any failures before proceeding.

**Step 7 –** Click **Next**.

**Step 8 –** Enable the scan types you want to run:
...
- Select **Include file-level permission data** to collect permissions at the file level in addition to folder level.
...
- Select **Run OCR** to scan images, screenshots, and scanned documents for sensitive text. This increases processing time.
