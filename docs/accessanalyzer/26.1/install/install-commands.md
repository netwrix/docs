...
| `DSPM_HOSTNAME` | `--hostname` | `aa26.1.corp.example.com` |
| `TARGET_REVISION` | `--target-revision` | `1.0.8` (pinned) or omit for latest |
| `SIZE` | `--size` | `small`, `medium` (default), `large`, `enterprise` |
| `TLS_CERT_FILE` | `--tls-cert` | `/opt/dspm-tls/aa26.1.crt` |
| `TLS_KEY_FILE` | `--tls-key` | `/opt/dspm-tls/aa26.1.key` |
...
For the full list of required network domains, see [Network and Port Requirements](/docs/accessanalyzer/26_1/install/system/network).
