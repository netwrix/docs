...
There's a reason why PolicyPak Application Settings Manager has a lot of log files:
PolicyPak Application Settings Manager supports both Group Policy and file-based
delivery methods. Therefore, if you're trying to troubleshoot PolicyPak Application
Settings Manager, use these advanced tips to help you understand Switched mode policies and PolicyPak Application Settings Manager's policy processing as a whole.
...
Additionally, any exported XML data files for PolicyPak Application Settings Manager
must be processed, even when there are no GPOs with PolicyPak Application Settings
...
During computer start-up, Windows checks if there is any PolicyPak Application
...
The user enters their credentials. Windows checks for any PolicyPak Application
