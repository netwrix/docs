...
provides choices for how you can export the report results for an investigation. You can export the
report in a specified format and download, email, or schedule it.
...
The application downloads reports to the Downloads folder on your local machine, according to your
browser settings. You can configure a folder on the application server to place copies of all exported
reports.

See the [Folder Settings Page](/docs/threatmanager/3.3/administration/configuration/integrations/foldersettings.md) topic for
...
investigation resides in the folder where you saved it.

**Step 2 –** After running a query and confirming the report sections display the data,
click the **Export** menu and select **Send as Email**. The Send as Email window opens.
...
Available email addresses from Active Directory that match the text string populate the
...
:::note
This option requires a configured shared folder. If this requirement isn't met, a
message will appear in the window. See the
[Folder Settings Page](/docs/threatmanager/3.3/administration/configuration/integrations/foldersettings.md) section for additional
information.
:::
...
**Step 2 –** After running a query and confirming the report sections display the data,
click the **Export** menu and select Scheduled export. The Schedule export window opens.
...
`{{Investigation_Name}}`. The application replaces this variable with the name of the investigation.
The + button opens a variable menu for the field.

**Step 5 –** Select a shared folder from the **Publish folder** dropdown menu. This menu only lists
folders configured on the Folder Settings page of the Integrations interface.
