...
Licensing Netwrix PolicyPak is easy. We have one tool to request
...
- PolicyPak is licensed on a per-client-computer basis. All client computers (desktop,
  laptop, and virtual desktop infrastructure [VDI]) must be licensed if you want PolicyPak policies to apply and enforce settings.
- PolicyPak is also licensed per concurrent-session-connection for your Terminal
  Services (RDS) or Citrix servers. All inbound Terminal Services (RDS) or Citrix XenApp connections
  must be licensed if you want PolicyPak to apply and enforce settings.
- PolicyPak can also be licensed in conjunction with an MDM service such as Intune,
  Workspace ONE, or MobileIron. This scenario is covered later in this guide and in detail in
  Appendix A: Using PolicyPak with MDM and UEM Tools.

:::note
To be in compliance with PolicyPak licensing, you must license inbound
Terminal Services (RDS) or Citrix connections. These are simply added to the count of your PolicyPak licenses. For instance, if you want to use PolicyPak with 500 desktops,
200 laptops, and 100 concurrent Terminal Services or Citrix session connections, then you will need
800 PolicyPak licenses. Full details of how PolicyPak licenses Terminal
...
PolicyPak on your Terminal Services (RDS) or Citrix machines, then these inbound
...
records in Active Directory. For most customers, all areas of PolicyPak licensing are
handled by the PolicyPak licensing tool (named LT.exe), which is included in the
Licensing for All On-Prem Products folder inside the ISO download.

PolicyPak licenses themselves can be deployed within Group Policy Objects (GPOs), or
delivered via MSI. Client computers download the GPO with the license file or the MSI with the
license file, and automatically determine if they are licensed. Be aware that the PolicyPak licensing tool (LT) is used with the Active Directory solution method and Intune, and not
...
:::note
To be in compliance with PolicyPak licensing on Citrix and Terminal Services
...
PolicyPak's LT utility can also help you minimize costs by finding and disabling
unused computers. For instance, many organizations have dozens or hundreds of computers that have
not been logged onto for months. Since you pay on a per-computer basis with PolicyPak
...
- Phase I: Get a License Request Key.In this phase, LT will ask several questions—such as where you
  ...
  good for one year, depending on your contract with PolicyPak.
- Phase II: Re-run and get another License Request Key. Once the initial license year is up, you
  simply re-run the PolicyPak LT utility each following year. LT will review existing
  installed licenses and do a count of computers. You need to perform this step every year
  (according to your PolicyPak license agreement) to continue to use PolicyPak. Otherwise, PolicyPak will stop functioning on your anniversary date. You
  pay for any overage from the previous year.

## Finding and Disabling Unused Computers

We only want you to pay for the computers you're actually going to use with the PolicyPak. PolicyPak's LT has an option, "Find and Disable unused computers," as seen in
Figure 12.

![licensing_policypak](/images/policypak/licensing_endpointpolicymanager.webp)
...
![licensing_policypak_1](/images/policypak/licensing_endpointpolicymanager_1.webp)
...
:::note
PolicyPak's LT utility cannot re-enable accounts once they've been disabled.
Also, PolicyPak's LT utility cannot move computer accounts after they are disabled.
:::


## Licensing Modes

Licensing with PolicyPak involves the following items:
...
- For PolicyPak Cloud: licensed by PolicyPak Cloud
...
- Licensed Mode (temporary). When a computer is licensed (temporarily for 30 days or licensed for
  ...
  no limit to the number of PolicyPak directives a client machine will process and
  keep compliant.
- Trial Mode. Trial Mode functions similarly to Licensed Mode. Trial Mode enables you to try
  PolicyPak very quickly in your test lab or real Active Directory. Trial Mode is
...
We want you to use this PolicyPak in your testing similarly to how you would use it in
...
These restrictions allow you to use your own test lab or Active Directory OU to try the PolicyPak components. You'll be able to see what happens when you make central changes and
watch your clients react.

## Licensing PolicyPak with Your Own MDM Service

PolicyPak can be used in conjunction with your own MDM service, such as Workspace ONE
or MobileIron. However, licensing PolicyPak with these utilities is a bit different
than licensing PolicyPak with Active Directory or Intune. In short, you need to work
...
what to send back to PolicyPak Sales can be found here:
...
![licensing_policypak_2](/images/policypak/licensing_endpointpolicymanager_2.webp)
...
save the file, and then deliver this to your PolicyPak sales representative to receive
a license file.
...
You may receive multiple license files for PolicyPak:
...
PolicyPak Cloud has a licensing mechanism build in. When a computer acquires a
license, it stays licensed unless it becomes unused for an amount of time. (See Appendix E: PolicyPak Cloud Quickstart and User Guide for more details on this.) However, there is one
caveat around a licensing scenario in which an acquired PolicyPak Cloud license could
possibly enable the Active Directory method. Below is the breakdown of how this works.

### Legacy PolicyPak Cloud monthly or yearly customers:

When clients consume licenses from PolicyPak Cloud, they automatically pick up Group
Policy as well (for free). So if you're a PolicyPak Cloud customer, you don't need to
...
with PolicyPak Cloud (and continues to check in within the check-in period). For
...
will not be able to automatically enable the Group Policy method when being licensed via PolicyPak Cloud.

### PolicyPak SaaS/Cloud-only customers:

In this case, you cannot use PolicyPak Cloud to enable the Group Policy method. If you
wish to enable the Group Policy method, you need to transition from PolicyPak SaaS to
PolicyPak Enterprise Edition or PolicyPak Professional Edition. You can
still manage Active Directory joined machines, but you must use the PolicyPak Cloud
delivery mechanism to perform the operation, and not Active Directory or a GPO.

## Final Licensing Thoughts

Once you've tested PolicyPak, you'll be ready to become a part of the licensed
PolicyPak family. The following are a few reminders on licensing:

- PolicyPak is licensed on a per-computer basis.
- When PolicyPak is fully licensed, the restriction on the computer name is lifted.
- The PolicyPak client-side extension (CSE) will not function unless it's in Trial
...
unlicensed, PolicyPak stops working. When you are licensed, it will fully process all
PolicyPak directives.

Volume licenses and domain-wide licenses for PolicyPak are available.
