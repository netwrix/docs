...
### knowledge_article_id and title-format rules

Both live in `.claude/references/kb-editing-conventions.md` — read §13 Frontmatter for the `knowledge_article_id` policy (presence/absence is informational, format is corrective, never propose adding/removing the field) and §12 Titles for the mechanical/semantic title-format split (gerund rules, title case, log-dump normalization, the two `Error:` prefix exemptions, H1/`sidebar_label` consistency, and the semantic-reframe soft-reminder rules). Apply mechanical fixes per the normal fix loop; semantic reframes are always the TSE's call — flag, suggest alternatives, defer.
