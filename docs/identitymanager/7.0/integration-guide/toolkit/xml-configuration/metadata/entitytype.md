...
| Identifier required | **Type:** String **Description:** Unique identifier of the entity type. It must is be unique to the *entity model* scope. Can't be a [reserved identifier](/docs/identitymanager/current/integration-guide/toolkit/reservedidentifiers). |
...
Some configuration elements will be linked to an entity whose id isn't known at configuration time. In this case, another key must be used. On each entity type property, the `IsKey` attribute specifies that the property can be used as a key during configuration import.
...
All *Title* instances will be replicated from a managed system. So, at configuration time, Identity Manager's internal primary key for this *Title* isn't known.

Therefore, you can't write a *SingleRoleRule* with a Dimension criteria based on *Title* as the primary key.

However, use a non-primary key that is known in advance, because it depends on the managed system's data and not on Identity Manager.
...
- Use `DateOnly` for calendar dates where timezone isn't relevant (birth dates, anniversary dates, holiday dates)
- Use `DateTime` for timestamps or events where time and timezone matter (contract start/end times, last login)

**Note:** The `AddedMinutes` configuration in display properties isn't applicable to `DateOnly` properties. If a property is changed from `DateTime` to `DateOnly`, any existing `AddedMinutes` configuration will be automatically ignored during import and removed during export, with warnings displayed to the user.
...
As long as the property wasn't used in any workflow, this can be properly handled by `Deploy-Configuration.exe`. If it *was* used in one or more workflows, foreign key conflicts (in UW_Changes database table) may occur, preventing the configuration from being deployed.
...
|Property|Details|
|---|---|
| DisplayName_L1 optional | **Type:** String **Description:** Display name of the property in language 1 (up to 16). **Note:** can't be "Id". |
