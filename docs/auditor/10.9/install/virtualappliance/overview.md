...
In addition to on-premises deployment, Netwrix Auditor offers a deployment option that can speed
time-to-value by getting Auditor operational in less than 15 minutes.
...
- [Import Virtual Machine from Image to VMware](/docs/auditor/10.9/install/virtualappliance/importvmware.md)
- [Import Virtual Machine from Image to Hyper-V ](/docs/auditor/10.9/install/virtualappliance/importhyperv.md)
...
- Consider Microsoft limits for evaluation versions of Windows Server. If your deployment is
  ...
  it gets a 180-day trial.
    ...
    Also, you can register your license by phone. See the following Microsoft article:
...
The following requirements are sufficient for evaluation purposes only. See the
[Requirements](/docs/auditor/10.9/requirements/overview.md) topic for additional information.

### Supported Platforms

The following table lists supported virtualization platforms for the virtual appliance deployment:
...
When you deploy Auditor virtual appliance, Auditor creates a pre-configured virtual machine from
the template. The following table lists the default hardware configuration for the VM where you
plan to deploy the virtual appliance:
