...
You can use command line options to install the Agent silently. This is useful with custom batch files, login scripts, and third-party software distribution solutions that your organization may already use.
...
| ------------ | ---------------- | --------------------- |
| CUSTOM_CA               | If this property is set to True, then the custom-managed certificate mode is enabled. This mode uses certificates that are signed by the customer's external certificate authority. In this mode, the installer won't generate certificates and won't start the Agent Service at the end of the installation. | FALSE             |
