...
encryption standard TLS 1.2** is used here. It's also possible to create a certificate via the
...
Communication between the client and server can only take place using the path that's stored in the
...
If the Netwrix Password Secure certificate isn't rolled out, it's also possible to manually import
...
[Basic configuration](/docs/passwordsecure/9.1/configuration/servermanager/basic_configuration.md).
A server certificate for SSL encryption is used here. The CA must be configured so
that all clients trust the certificate. It's necessary to adhere to the certification path.
