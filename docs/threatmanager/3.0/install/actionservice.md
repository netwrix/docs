...
downloaded from within the Threat Manager Console.
 ...
 install the Action Service.

![install](/images/threatmanager/3.0/install/install.webp)

**Step 1 –** Run the StealthDEFEND.ActionService MSI installation package. The Threat Manager
Action Service Setup wizard opens.
...
server as an Action Service server. Perform this registration by providing the Threat Manager
web console location and credentials via a command line utility. This process registers the host
as an Action Service Server and retrieves a JWT that is encrypted and stored locally in the
...
fetches queued actions from Threat Manager to execute locally.
 ...
 configure the Action Service using the command line utility.
...
- User Name – Specify the user name to connect to the Threat Manager console. Use the Admin account for the user name.
- Threat Manager Password – The password to the user name specified
- Ignore certificate errors – Set to True if using a self-signed certificate for SSL or if other issues with the web certificate are experienced.

**Step 4 –** The utility outputs "Success!" if the Action Service registers correctly. Verify
...
The Action Service installed with Threat Manager can be configured to execute as a service
account. In this scenario, Threat Manager executes any actions as the service account. This approach ensures that playbooks have the proper permissions required to complete successfully.
...
:::
 ...
 configure the Action Service to run as a service account.

**Step 1 –** Open Services (`services.msc`).

![services](/images/threatmanager/3.0/install/services.webp)

**Step 2 –** Double-click the Netwrix Threat Manager Action Service. The Threat Manager Action
...
**Step 4 –** Select the This account: radio button and specify the Active Directory account
...
Once restarted, the Threat Manager Action Service uses the specified service account.
