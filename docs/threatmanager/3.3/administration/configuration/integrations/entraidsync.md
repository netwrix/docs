...
:::note
Before adding a Microsoft Entra ID Sync policy, you must first configure a Credential
...
[Application Server Requirements](/docs/threatmanager/3.3/requirements/server.md) topic for the permissions. See
the [Credential Profile Page](/docs/threatmanager/3.3/administration/configuration/integrations/credentialprofile.md) topic for additional information on creating a
profile.
:::
...
To view the Microsoft Entra ID Sync policy details, select the tenant from the table or
the navigation pane.
...
**Step 1 –** On the Integrations interface, select the Microsoft Entra ID tenant.
