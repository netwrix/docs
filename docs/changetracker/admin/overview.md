---
title: "Administration"
description: "Administration"
sidebar_position: 50
---

# Administration

The Change Tracker console contains several pages, which you select from the tabs at the top:

- Dashboard Tab
- [Events Tab](/docs/changetracker/admin/events/events.md)
- [Device Tab](/docs/changetracker/admin/devices.md)
- [Overview Tab](/docs/changetracker/compliance/complianceoverviewtab.md)
- [Planned Changes Tab](/docs/changetracker/admin/plannedchanges/plannedchanges.md)
- [Reports Tab](/docs/changetracker/admin/reportstab/reportstab.md)
- [Settings Tab](/docs/changetracker/admin/settingstab/settingstab.md)
