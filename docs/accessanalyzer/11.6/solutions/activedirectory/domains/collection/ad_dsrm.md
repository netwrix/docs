...
the DSRMAdminLogonBehavior key. If this key is set to 1 or 2, the Directory Services Restore Mode
(DSRM) Admin Account can log in to the domain controller even if it has not been started in DSRM,
which can present a potential security vulnerability. Additional information on this registry key
...
The AD_DSRM Job uses the Registry Data Collector for the following query:
