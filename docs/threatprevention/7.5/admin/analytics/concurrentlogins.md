...
| Example              | A user doesn't normally login from multiple locations simultaneously within a short time window. If one or more accounts has been compromised by malware, or during an insider attack, one might see a compromised account authenticating from many systems in parallel and a corresponding analytic alert identifying the account and the authenticating systems. |
