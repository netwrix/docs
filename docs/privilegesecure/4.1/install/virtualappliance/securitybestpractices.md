...
- don't create additional local administrator accounts on the Privilege Secure server unless
absolutely required. If created, ensure their passwords are long enough (>20 characters), not
...
organization trusts.
- ensure that the clocks of the virtual machine where the appliance deployed is synced to an NTP
Server, Netwrix recommends the same NTP source as your domain controllers are synced to.
- Ensure Windows Updates are enabled and automatically installed with a plan to manually reboot
created. don't enable automatic reboot.
