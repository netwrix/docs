...
:::warning
Don't install the File System Proxy Service on the same server as Access
Analyzer.
:::
...
where Access Analyzer deploys the applet initiates the data collection processing and runs a
...
The diagram illustrates the Access Analyzer server sending a FileSystemAccess (FSAA) applet to a
proxy server, which runs the scan against a file server, and then returns data to the Access
Analyzer server.
...
When you initiate a proxy mode scan from the Access Analyzer Console, it distributes the target
hosts across all proxy hosts. Access Analyzer monitors the scans from the central console. Once
all proxy hosts have completed scanning, they return all results and SQLite databases to the Access
Analyzer Console server.
...
Collector scans. This lets it execute three times as many concurrent hosts as would be
...
run two concurrent threads. For example, a job configured to scan 8 hosts at a
