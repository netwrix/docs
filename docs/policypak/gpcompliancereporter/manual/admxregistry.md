...
PolicyPak GPCR has the ability to provide extended logging if something unexpected
...
[Knowledge Base](/docs/policypak/). The ADMX files are
in the download of PolicyPak GPCR, as shown in Figure 77.

![tuning_and_troubleshooting_18](/images/policypak/troubleshooting/grouppolicycompliancereporter/tuning_and_troubleshooting_18.webp)

Figure 77. Downloaded ADMX files.

## GPCR Downloadable REG Files

You can use the downloadable REG files when directed by the PolicyPak Technical Team.
In this case, you would typically be troubleshooting the following:

- Endpoint
- PolicyPak GPCR client (admin console)
- PolicyPak GPCR server
...
![tuning_and_troubleshooting_19](/images/policypak/troubleshooting/grouppolicycompliancereporter/tuning_and_troubleshooting_19.webp)

Figure 78. Enhanced logging enabled.

## ADMX/ADML files

You can enable diagnostics for the PolicyPak GPCR family of products using the
...
| Administrative Templates | PolicyPak, as shown in Figure 79.

![tuning_and_troubleshooting_20](/images/policypak/troubleshooting/grouppolicycompliancereporter/tuning_and_troubleshooting_20.webp)
...
settings that are not enabled by Group Policy. For the PolicyPak GPCR client (admin
...
For the PolicyPak GPCR server the following is not enabled by Group Policy:
...
for the PolicyPak GPCR server. The target location specified must have network service
...
| --------------------------------------------------------------------------- | ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Endpoint                                                                    | Enable diagnostics output for PolicyPak GPCR auditor | Key: `HKEY_LOCAL_MACHINE\Software\Wow6432Node\Policies\PolicyPak\PPGPCR Auditor Endpoint` Values: `EnableDiagnostics`, `DiagnosticsPath` | `%ProgramData%\PolicyPak\PolicyPak Group Policy Compliance Reporter Auditor Endpoint\Diagnostics` |
| PolicyPak GPCR client (admin console)                         | Enable diagnostics output for PolicyPak GPCR         | Key: `HKEY_LOCAL_MACHINE\Software\Wow6432Node\Policies\PolicyPak\PPGPCR Client` Values: EnableDiagnostics, DiagnosticsPath               | `%LOCALAPPDATA%\PolicyPak\PolicyPak Group Policy Compliance Reporter\Diagnostics`                 |
| PolicyPak GPCR server                                         | Enable diagnostics output for PolicyPak GPCR server  | Key: `HKEY_LOCAL_MACHINE\Software\Wow6432Node\Policies\PolicyPak\PPGPCR Server` Values: `EnableDiagnostics`, `DiagnosticsPath`           | `%ProgramData%\PolicyPak\PolicyPak Group Policy Compliance Reporter Server\Diagnostics`           |
