...
Then retry the Netwrix PolicyPak operation.

:::note
Troubleshooting Group Policy Replication problems is beyond the normal PolicyPak tech support, since it's not specifically a PolicyPak issue. If you have a
...
the scope of the PolicyPak troubleshooting support team.
:::


Symptoms: Everything might be running smoothly until you get inconsistent results upon clients when
your PolicyPak directives embedded within GPOs. Some of your workstations are
receiving the GPOs others aren't.

While we don't want to completely rule out PolicyPak's engine, it's more likely an
...
![3_1_troubleshooting-grou-policy-replication2](/images/policypak/troubleshooting/applicationsettings/3_1_troubleshooting-grou-policy-replication2.webp)
...
It's very possible the first GPO to express an error one that contains PolicyPak
...
![3_2_troubleshooting-grou-policy-replication1](/images/policypak/troubleshooting/applicationsettings/3_2_troubleshooting-grou-policy-replication1.webp)
...
- GPOTOOL.exe – This powerful CLI tool checks the consistency of Group Policy Objects (GPOs) between
  ...
  PolicyPak client. A review of the logs showed that both PolicyPak and
  Non PolicyPak GPOs were having issues. This confirmed that the problem was beyond
  the scope of just PolicyPak. Further review of the log file showed that there was a
