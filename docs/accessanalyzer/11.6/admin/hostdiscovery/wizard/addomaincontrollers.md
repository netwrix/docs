...
This query only requires an authenticated domain user account — you don't need an elevated Active
Directory administrative role (such as Domain Admin or Enterprise Admin) to enumerate domain
...
:::info
Domain Controllers are a high-value target class. If you enable **Refresh inventory every time the
host discovery query completes** on the Inventory page, the account used for inventory needs
