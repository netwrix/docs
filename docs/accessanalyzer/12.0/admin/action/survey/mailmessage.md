...
- Load from template – Survey templates are a legacy feature. Netwrix strongly recommends not using
...
- Save to template – Saves the current email subject and content to a template. If an existing
  ...
  existing templates, clicking this button accesses the Save SendMail Template window, and you can save changes
  to a new template. Templates reside locally on the host computer as XML files, in the
