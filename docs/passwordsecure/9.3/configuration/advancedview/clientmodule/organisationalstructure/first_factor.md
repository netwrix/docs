...
That `{username}` for username and `{domain}` for the AD domain SHOULD be present in the
...
SMTP ([Advanced settings](/docs/passwordsecure/9.3/configuration/servermanager/mainmenu/advanced_settings.md)) is configured.
