...
![understanding_processing_order](/images/policypak/remoteworkdelivery/understanding_processing_order.webp)
...
Netwrix PolicyPak Remote Work Delivery Manager will simply merge
all PolicyPak Remote Work Delivery Manager policies that come from the Group Policy
...
PolicyPak Remote Work Delivery Manager policies can be delivered by Group Policy and
non-Group Policy methods such as MEMCM (via PolicyPak Exporter or PolicyPak MDM) or PolicyPak Cloud. As such, the PolicyPak Remote Work
Delivery Manager engine needs to make a final determination regarding whether there is any conflict
between PolicyPak Remote Work Delivery Manager policies (which are essentially rules).
Here is how precedence works:

- Polices delivered by PolicyPak Cloud have the lowest precedence.
- PolicyPak file-based policies (including those delivered from an MDM service) have
  the next highest precedence.
- PolicyPak Group Policy policies have the highest precedence.
