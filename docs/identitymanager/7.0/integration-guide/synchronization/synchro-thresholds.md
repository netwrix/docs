...
If launch synchronization for an entity type whose threshold values are **lower than the connector's**, then Identity Manager blocks synchronization as soon as the number of modifications exceeds the entity type's threshold values.
