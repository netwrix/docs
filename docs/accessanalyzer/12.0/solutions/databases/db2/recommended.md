...
The Db2 Solution inherits settings from the **Db2** > **Settings** node. However,
...
The Db2 Job Group inherits the host list assignment from the collection group
level.

Assign the host list under the **Databases** > **0.Collection** > **Db2** >
...
Assign the Connection Profile under the Databases > 0.Collection > Db2 > Settings >
Connection node. By default, the connection node uses the Default Profile configured at the global
settings level.
...
Run the 0.Collection Jobs first and in order. You can run the other Db2 sub-job groups in any
...
2. Configure and assign host list and connection profiles
