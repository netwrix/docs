...
Once the designated server is licensed and the PolicyPak GPCR client (admin console)
...
server. See Figure 42 for a diagram of how PolicyPak GPCR Server with push mode
receives information from PolicyPak GPCR endpoints.

![gpcr_server_with_push_mode_6](/images/policypak/grouppolicycompliancereporter/mode/push/gpcr_server_with_push_mode_6.webp)
...
:::note
This can be any server at all and not necessarily the same server running the PolicyPak GPCR server.
:::


**Step 3 –** Data (the RSOP report) is generated on the client and processed for delivery.

**Step 4 –** About one minute later, data is pushed from the PolicyPak GPCR endpoint
to the designated server.

Data is then stored in the database, where PolicyPak GPCR clients (admin consoles) can
