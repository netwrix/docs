...
ensure that Managed Service Account is selected in the User Credentials window. See the
...
To run a job or scheduled task with a gMSA/MSA account, meet the following prerequisites:
...
- The gMSA account doesn't have to be a local admin in the Access Analyzer Console
...
:::note Action Modules
Access Analyzer doesn't support using action modules with gMSAs.
:::
