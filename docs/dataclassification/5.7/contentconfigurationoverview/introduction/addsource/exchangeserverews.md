...
The Exchange Server (EWS) source configuration screen allows you to enable the crawling and classification
of multiple Exchange mailboxes from the same Exchange server using the Exchange Web services (EWS).
...
Follow the steps to configure an Exchange Server (EWS) source.
