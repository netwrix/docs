...
**IMPORTANT:** By default, ONTAP already has a service policy configured and applied to the managed
SVM LIFs. If you want to use OAuth 2.0 authentication instead of AD domain authentication, skip the
steps in this topic.
...
```
network interface show -vserver svm1 -fields service-policy
```
Example output:
...
command:
```
network interface service-policy show -vserver svm1 -policy default-data-files
```
Example output:
    Vserver: svm1
...
```
network interface service-policy clone -vserver svm1 -policy default-data-files -target-policy netwrix-policy -target-vserver svm1
```
...
following command:
```
network interface service-policy add-service -vserver svm1 -policy netwrix-policy -service management-http
```
```
network interface service-policy add-service -vserver svm1 -policy netwrix-policy -service management-https
```
...
```
network interface modify -vserver svm1 -lif lif_svm1_126 -service-policy netwrix-policy
```
```
network interface modify -vserver svm1 -lif lif_svm1_349 -service-policy netwrix-policy
```
...
where `netwrix_policy` is your Firewall policy name and `192.168.1.0/24` is your subnet where Netwrix Auditor Server resides.

4. Apply the firewall policy to a LIF.
```
network interface modify -vserver svm -lif vs1-cifs-lif1 -firewall-policy netwrix_policy
```
