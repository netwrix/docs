...
If you want to know which GROUPS your Users or Computers are in within Azure AD, then use Netwrix PolicyPak and these scripts. Then after that, you can use Item
...
There's also a second video for using this in PolicyPak Cloud, but don't do that one
...
in PolicyPak and Group Policy land, read the key, do some magic, and pull back out the
Lookup password.  
...
Now, again, if you only care about PolicyPak Cloud, you can maybe pause the video at
...
computer side first. If I go to computer side, "PolicyPak," "Scripts Manager," what
...
that's fine. What we're going to do is on the user side in "PolicyPak" "Scripts
...
Policy Preferences or an Admin Templates item or a PolicyPak item, how do you take
...
just in "Preferences," of course, but anywhere in PolicyPak land. So if you wanted to
...
Okay, so that ends this first video. If you want to do this in PolicyPak Cloud, you
