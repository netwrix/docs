...
- **Title case correction** (e.g., "configure stopwords" → "Configuring Stopwords"). Applies to all article types. **When the body has an H1, defer to Vale — do not duplicate the finding.** `NetwrixKB.HeadingCase` (`scope: heading`) already fires on the H1, so this is Vale's finding, not a second Derek/rulebook-area row for the same title; skills that run both Vale and this rulebook area in the same report (e.g., `kb-pr-open`, `kb-pr-review`) must not flag the same H1 twice. **When the body has no H1 at all (§13's title row), this deferral does not apply — check the frontmatter `title` value's case directly,** since Vale's heading-scoped rule never evaluates a frontmatter field and nothing else will catch a badly-cased title on those articles.
