...
Since these policies aren't integrated; you should either useMicrosoft Entra ID’s or Directory
Manager’s expiration policy settings for groups in an Microsoft Entra ID identity store.

## Group Naming Policy

The group naming policy defined in Microsoft Entra ID isn't integrated with the Group Name Prefixes
...
- Using Directory Manager, you can't modify the prefixes and suffixes that are applied to groups
...
Don't manipulate the same object simultaneously from Directory Manager and Microsoft Entra Admin
Center.

## Multifactor Authentication Policy

The multifactor authentication (MFA) policy defined for Microsoft Entra Admin Center can't be
...
in Directory Manager, they can manipulate directory objects using Directory Manager that they can't otherwise manipulate directly in Microsoft Entra Admin Center due to limited role
permissions.
...
has in Microsoft Entra Admin Center. For example, a user who doesn't have permissions to create
...
- To create security groups and Office 365 groups using Directory Manager, Office 365 must be set as
  the messaging provider for the identity store. Security groups can't be mail-enabled. Use an
  Office 365 group as an alternative to a mail-enabled security group.
- Microsoft Entra ID doesn't allow a group to be added to the membership of an Office 365 group.
  For this reason:
    ...
    - The nesting option in the _Out of Bounds_ settings for an identity store will empty the
      membership of a Smart Group of the Office 365 type, because nested groups can't be added as
    ...
    - A Dynasty can't be created as an Office 365 group.

- You can create and manage distribution groups.
- You can also create teams and channels in MS Teams.
- You can't create Office 365 connected Yammer groups and Planner groups using Directory Manager.
...
- When you expire an Office 365 group using Directory Manager, its member list is backed up in the
  database and then cleared from Office 365. The Microsoft Graph API doesn't allow for the
  modification and removal of the mail attribute; therefore, it isn't possible to remove the
  group’s email and make it mail-disabled.
- Directory Manager can't manipulate directory-synced objects (objects that are synced from
...
- You can create users (non mail-enabled) and mailboxes in a Microsoft Entra IDidentity store. The
  contact object type isn't supported.
- The Photo attribute is only available for an Office 365 user (mailbox).
- The Directory Manager portal doesn't display the password expiry date of a user account in a
...
- For a Microsoft Entra ID identity store, you must configure Office 365 as the messaging provider
  and SMTP server.
- The Recycle Bin doesn't display any data for a Microsoft Entra ID identity store.
- Directory Manager Management Shell can't communicate directly or remotely with Microsoft Entra ID
  and Office 365.
...
- The company attribute in Microsoft Entra ID stores the name of the company registered in Microsoft
  Entra Admin Center, and can't be manipulated using Directory Manager. A Microsoft Entra ID
