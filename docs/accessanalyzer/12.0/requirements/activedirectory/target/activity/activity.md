...
Set up integration between Activity Monitor and Access Analyzer through an API
server.
...
Configure the agent deployed to the API server.
...
- The default **API server port (TCP)** is 4494, but you can modify it if desired. Ensure Access
  Analyzer also uses the modified port.
...
- Configure the following:
    ...
    - By default, the **Secret Expires** in 3 days. That means you must use it in the Access
      Analyzer Connection Profile within 72 hours, or you will need to generate a new secret. Modify
      if desired.
...
:::tip
Remember, the API server generated the Client ID and Client Secret, and you copied them to a text
file. If the secret expires before you configure the Connection Profile, you will need to
re-generate it.
:::
...
Access Analyzer now targets the Activity Monitor API Server with this query to collect domain
activity logs.
...
Center. However, this is disabled by default. Enable the
import of AD activity data into the Access Information Center.
...
You have now configured the **AD_ActivityCollection** Job to import both AD events and
authentication events into the Netwrix Access Information Center.
