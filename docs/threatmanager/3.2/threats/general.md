...
| Trigger               | Threat Manager analyzes user behavior over a learning period (e.g. 30 days) to profile which clients a user normally uses. After a new client is used to perform file system activity for the first time for a particular user, Threat Manager creates a threat. |
