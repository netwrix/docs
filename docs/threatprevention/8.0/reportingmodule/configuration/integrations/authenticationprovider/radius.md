...
![Integrations interface displaying the details for a Radius authentication provider](/images/threatprevention/8.0/reportingmodule/configuration/integrations/authenticationprovider/radius.webp)
...
![Configuration tab for a RADIUS authentication provider](/images/threatprevention/8.0/reportingmodule/configuration/integrations/authenticationprovider/configurationradius.webp)
...
- Default – The default profile applied when a user is assigned multiple authentication profiles.
  ...
  on as desired.
- Server FQDN/IP – The fully qualified domain name (FQDN) or IP address of the RADIUS proxy
...
- Auth Type – The security protocol used by the RADIUS proxy. Use the dropdown menu to select
  either MS-CHAP version 2 (MSCHAPv2) or Password Authentication Protocol (PAP).
- Shared Secret – A secret shared between the application server and the RADIUS proxy
- User Name Format Type – Active Directory attribute or attributes you send to the RADIUS
...
![Customization tab for a Radius authentication provider](/images/threatprevention/8.0/reportingmodule/configuration/integrations/authenticationprovider/customizationtab.webp)

- Title for MFA Authentication dialog – The title displayed to the user when prompted for
  MFA
- Text for MFA Authentication dialog – The text description displayed to the user when
  prompted for MFA
- Send Initial Text – If On, the application automatically sends the value in the Initial Auto Response Text to
  the RADIUS proxy without user action. Toggle On or Off as desired.
- Initial Auto Response Text – The application sends this value to the RADIUS server automatically if the Send
...
- Prefix for Response Text – The application adds this value to the start of responses. The value varies
  according to server.
- Send NAS Identifier – When On, the application transmits Network Access Server (NAS) identifiers to the RADIUS proxy. This is needed
