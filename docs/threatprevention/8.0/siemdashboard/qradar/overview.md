...
complete the following steps to install it within QRadar.
...
Complete the following steps to configure QRadar to receive data from Threat Prevention.
...
- Log Source Type – Enter one of the following options:
  ...
  - Threat Prevention Analytics
    ...
    (Set up three separate log sources, with each one using one of these types.)
