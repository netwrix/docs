...
- The priority of user detection in the login flow has been updated. Users coming from Entra ID are now prioritized over local users having the same email address configured.
- Security: The way JSON-serialized values are deserialized has been improved. ([ADV-2026-005](https://community.netwrix.com/t/adv-2026-005-vulnerability-in-netwrix-password-secure/125724))
...
### Browser extension
- Support for the Safari extension (MacApp) has ended. New versions of this application are no longer published. Use supported browsers like Microsoft Edge, Google Chrome, or Mozilla Firefox to continue using the automatic password entry in websites.
