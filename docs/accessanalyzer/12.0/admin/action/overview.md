...
Access Analyzer actions can change users, permissions, files, and objects from a
...
Actions with the checkbox next to their name selected in the Action Selection view run
automatically when the job runs. The actions run in the order in which they
...
Exercise caution to ensure the action applies only the changes you want and only to the
...
only the data you want. Also run the action in a test environment before making
...
- Data Grid – Displays a sample of the selected Source table. This data grid functions the same as
  all data grids within Access Analyzer. You can filter data and regroup columns. See the
...
the action module being used. Otherwise, errors may occur when you run the action, affecting
...
:::tip
Remember, individual action modules may have their own column requirements in addition to the
columns listed here.
:::
...
Access Analyzer native data tables generally contain all the columns listed here. However, if all
