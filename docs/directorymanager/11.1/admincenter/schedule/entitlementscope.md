...
[GroupID Entitlement Schedule](/docs/directorymanager/11.1/admincenter/schedule/entitlement.md), as the latter replicates all permissions.
However, the default triggering frequency for the Directory Manager Entitlement schedule (i.e.,
weekly) requires a separate Entitlement Scope schedule.
...
You can't create or delete an Entitlement Scope schedule; only edit the existing schedule.

## Update the Entitlement Scope Schedule

To update the Entitlement Scope schedule.
