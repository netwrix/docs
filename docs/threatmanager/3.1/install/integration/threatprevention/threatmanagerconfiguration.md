...
**Step 4 –** Optionally enable SSL for the Advanced Message Queuing Protocol (AMQP) event stream.

As Threat Manager supports TLS/SSL, you can also enable SSL in Threat Prevention to enable
communication with Threat Manager securely.

- If Threat Manager has TLS enabled, select the **Enable SSL** checkbox to enable the Agent to
...
When a Forged PAC analytic is triggered in Threat Prevention, the event data is sent to Threat
Manager.
