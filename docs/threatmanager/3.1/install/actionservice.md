...
as an Action Service Server and retrieves a JSON Web Token (JWT) that is encrypted and stored locally in the
...
- Ignore certificate errors – Set to True if using a self-signed certificate
...
account. In this scenario, any actions run by Threat Manager are executed as the Service
Account. This ensures that Playbooks executed by Threat Manager have
...
[Credential Profile Page](/docs/threatmanager/3.1/administration/configuration/integrations/credentialprofile.md) topic
for additional information.
:::


To configure the Action Service to run as a service account.
...
**Step 4 –** Select the **This account:** radio button and specify the Active Directory account and
credentials. If desired, select the Browse button to open the Select User window to select an
account.
