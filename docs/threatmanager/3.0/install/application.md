...
You can install the application on the same server as the PostgreSQL databases or on a different server. If installing on a different server, provide the location of the database server during installation.
...
same server where the PostgreSQL database application was installed, this information is
...
indicating that the installer creates these. Deselect the checkbox if you don't want the
...
post-installation tasks to complete, depending on your environment. See the
...
- During the first launch, set up the built-in Administrator account. See the
