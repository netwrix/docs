...
The right to review prolonged entitlements is given by adding the appropriate `AccessControlRule` on a profile. A profile should get the right to review prolonged entitlements given for both single and composite roles. Technically speaking, you must create one access control rule for assigned single roles, and another one for assigned composite roles. In this case, access is given to the workflow state 27, which is the workflow state `Prolonged` linked with the grace period.
