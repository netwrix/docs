...
While this suspends further Threat Prevention event monitoring and blocking on the affected domain controller, it ensures the domain controller continues to operate normally. After Netwrix deploys a counter patch, you can restart the Agent from the Administration Console. The console will first check if the Agent is set to ‘manual start’ mode and, if so, set it back to ‘automatic start’ mode and restart it, restoring normal operation.

:::note
Register a primary and a secondary technical contact for your organization with Netwrix. Netwrix will send notifications to these contacts about Microsoft-issued KBs affecting LSASS and relevant counter patches. Contact your Netwrix Sales Representative or [Netwrix Support](https://www.netwrix.com/support.html) to ensure that these contacts are registered.
:::


Before Microsoft releases a KB that alters the LSASS components in a way that conflicts with the Agent, Netwrix sends a notification to registered technical contacts with detailed instructions for obtaining and applying the Agent counter patch. Counter patches are typically made available 24 hours or more before Microsoft issues the incompatible KB.
...
from the dropdown menu. See the [Email Tab](/docs/threatprevention/7.5/admin/configuration/systemalerting/email.md) topic
