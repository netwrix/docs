...
## Set up integration between Activity Monitor and Access Analyzer through an API server
...
- Select the **Archive log files on a UNC path** option. Click the **...** button and navigate to
  the network you want share on the API server.
...
These agents are configured to save the Archive logs to the selected share.
...
- Configure the number of days you want for the **Period to keep Log files**. This is the number of
  days the log files are kept on the API server after configuration. This needs to be
...
**AD_ActivityCollection** Job. However, this is disabled by default.
