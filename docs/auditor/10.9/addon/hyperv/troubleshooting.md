...
- In Netwrix Auditor settings, go to the **Integrations** section and ensure the **Leverage
...
- If you deployed the solution using the third scenario (that is, SCVMM server and add-on are
  running on different machines), the solution may write the following error to the solution log:
...
a domain, use HTTPS transport or add the destination machine to the
