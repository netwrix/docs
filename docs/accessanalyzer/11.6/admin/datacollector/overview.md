...
You assign data collection tasks to jobs at the **Configure** > **Queries** node level. See the
...
Use the Queries section to configure the Data Collectors. The Queries section has the
following options:
...
You can add pre-built queries to the Data Collector job through the Libraries window.
...
- Create New Library – Create a new library entry. The new Library appears in the Library
  dropdown menu.
...
for an existing query. The Query Properties window has three tabs where you can configure queries.
These tabs are:
...
- ID – Query GUID generated by Enterprise Auditor for this query task.
    ...
    - The **SMARTLog** Data Collector, **ExchangeMetrics** Data Collector, and **PowerShell** Data
      Collector all reference the query GUID.
    - When using the **SMARTLog** Data Collector or the **ExchangeMetrics** Data Collector, you can
      find the `state` file for the query’s Persist log state feature in the SA_CommonData
...
all licensed data collectors in alphabetical order. The wizard auto-fills the **Query** and
**Property** sections according to the configuration. The buttons at the bottom of the Property section are
...
Click **OK** to save changes and exist the Query Properties window. If you didn't make or intend any
changes, click **Cancel** instead to exit the Query Properties window without saving unintended
changes.
