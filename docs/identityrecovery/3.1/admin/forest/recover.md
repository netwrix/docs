...
You can recover a forest using the domain controller backups generated by Identity Recovery. You use a restore playbook, also called a recovery playbook, to restore a forest. After you add the domain controllers you want to the restore playbook, you can initiate the forest recovery process, and Identity Recovery restores the forest using its automated process.
...
- Create a duplicate of a domain controller, for example, deploy a duplicate in a lab
...
See the [Forest Page](/docs/identityrecovery/3.1/admin/forest/forest.md) topic for additional information.
...
- **Step 1 –** Prepare an isolated environment to restore the forest to.
    ...
    See the [Target Server Considerations](/docs/identityrecovery/3.1/requirements/targetserver.md) topic to understand the requirements for a target environment and target servers for restoring domain controllers.

- **Step 2 –** Add the domain controllers you want to create a recovery playbook, also called a
  restore playbook.
...
After the domain controllers you want to restore have at least one backup to choose from, you can
start the forest recovery playbook by adding the domain controllers to it.

:::note
You only need one backup of a domain controller for each domain in the forest. You can then add any other
domains via the playbook or handle them manually using the "add DC to
the domain, let replication happen" model.
:::

To create a recovery playbook:
...
![Add to Recovery Playbook wizard - Domain Controller page](/images/identityrecovery/3.1/product/forest/recovery/forest_recovery_page1.png)

**Step 4 –** On the Domain Controller page, the Source Domain Controller section displays
information for the domain controller you're adding to the playbook. It displays the domain it
...
See the
[Target Server and Operating System Requirements](/docs/identityrecovery/3.1/requirements/targetserver.md#target-server-and-operating-system-requirements)
topic for additional information.

**Step 5 –** Click **Next**.

![Add to Recovery Playbook wizard - Options page](/images/identityrecovery/3.1/product/forest/recovery/forest_recovery_page2.png)

**Step 6 –** From the **Backup** dropdown menu, select the backup to use for restoring the domain
controller. The dropdown menu lists the backups available for the domain controller.
...
the domain controller you're restoring.

**Step 10 –** Click **Next**.

![Add to Recovery Playbook wizard - Confirm page](/images/identityrecovery/3.1/product/forest/recovery/forest_recovery_page3.png)
...
The wizard creates the playbook with the domain controller added to it.

Repeat this process for all the domain controllers you want to restore in the target environment.
...
![Recovery Playbook created on the Forest Page](/images/identityrecovery/3.1/product/forest/recovery/forest_recovery_finished.png)

The playbook displays the following information for a domain controller:

- Original Server – The name of the server that you added to the playbook for restore
- Target Server – The server where Identity Recovery restores the domain
...
- Roles – The FSMO roles the domain controller will acquire when restored
- Backup Time – the date and time of the backup that Identity Recovery restores for the domain controller

### Run the Recovery Playbook

When your playbook is ready, click **Start** to initiate the forest recovery process. The machine
will reboot after the recovery process is complete
...
![Forest page showing a restored forest](/images/identityrecovery/3.1/product/forest/recovery/forest_recovery_playbook_finished.png)
