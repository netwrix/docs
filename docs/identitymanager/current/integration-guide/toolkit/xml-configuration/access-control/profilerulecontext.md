...
| RootExpression optional | **Type:** String **Description:** C# expression evaluated on the resource being profiled. With `ResourceType`, the expression is evaluated on the target entity type of the resource type; with `EntityType`, on the entity type directly. [See more details on C# expressions](/docs/identitymanager/current/integration-guide/toolkit/expressions#c-expressions). |
| SubExpression optional | **Type:** String **Description:** C# expression evaluated on each resource reached via `SubBinding`. With `ResourceType`, the expression is evaluated on the source entity type of the resource type; with `EntityType`, on the entity type reached via `SubBinding`. [See more details on C# expressions](/docs/identitymanager/current/integration-guide/toolkit/expressions#c-expressions). |
