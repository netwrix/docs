...
Use the Target page to identify the host to get data from (wizard only) and the .admx source. It is
...
In the Use these job credentials to browse section of the page, if you've set up multiple
credentials, select the credentials to use for the query from the dropdown menu.

:::note
If the Default Connection profile has only one set of credentials, the dropdown grays out and
displays only the default credentials for that profile.
:::
