...
:::note
As a best practice, set the Connection Profile at the same level where you set the job’s
host list. For example, if you set the host list under the job group’s **Settings** node, configure
the Connection Profile there. If you set the host list under the **[Job]** >
**Configure** node, configure the Connection Profile there.
:::
