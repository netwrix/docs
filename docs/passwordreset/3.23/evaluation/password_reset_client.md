...
an automated deployment, so this guide covers manual PRC installation for the evaluation.
...
This lets you centrally configure all computers in the domain. It takes about 15 minutes to set
up the Administrative Template, so this guide covers PRC configuration by importing the settings into the
...
The client works on Windows XP and Server 2003, but the pages don't display correctly on these
