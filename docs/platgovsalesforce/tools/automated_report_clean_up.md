...
Clean up excludes the following reports:
...
- **Months Since Date Last Used, Retained** filters reports by the most recent activity date (last
  run date, retained, or restored). The filter excludes reports used within the specified timeframe.
  Default is **6** months.
...
- **Select/Selected Report Owners** filters reports by matching selected owners for the rule. If you
  check **Exclude Owners**, the rule excludes matching owners instead.
...
Use **Preview** to validate the rule and review the list of matching reports.
...
1. When a Report matches a Clean Up Rule:
   ...
   - The system emails a notification to report owners unless you disable that notification.
...
The Automated Report Clean Up tool includes Salesforce reports that administrators use to monitor
