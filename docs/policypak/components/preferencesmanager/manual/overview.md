...
- Optionally, if you don't want to use Group Policy, read the section in Appendix A:
  [Using PolicyPak with MDM and UEM Tools](/docs/policypak/gettingstarted/mdmmanual/uemtools/uemtools.md) on Group Policy and
  non–Group Policy methods (MEMCM, KACE, and MDM service or Netwrix PolicyPak Cloud) to deploy your directives.
...
![about_policypak_gpo_export](/images/policypak/preferences/about_endpointpolicymanager_gpo_export.webp)

Despite these advantages, Microsoft's Group Policy Preferences have some issues that cannot be
overcome without a little help. That's where PolicyPak Preferences Manager comes in.

## Main Concepts

PolicyPak Preferences Manager does the following jobs:
...
- It enables a Group Policy Preference item to be delivered without using Active Directory Group
  Policy. Therefore, with PolicyPak Preferences Manager, you may now use Microsoft
...
- In conjunction with the PolicyPak Cloud service, or your own mobile device
  management (MDM) service, you can deliver Group Policy Preference items to computers over the
  Internet (to both domain-joined and non-domain-joined machines). For more information on PolicyPak Cloud, see Appendix E:
  [Setup, Download, Install, and Verify](/docs/policypak/gettingstarted/cloudmanual/quickstart/quickstart.md) and User Guide. For more
  information about using PolicyPak with an MDM service, see Appendix A:
...
:::note
See
[Which settings can be managed with the Preferences Manager component?](/docs/policypak/components/admintemplatesmanager/knowledgebase/exportinggrouppolicypreferences/settings.md) for additional
information on PolicyPak Preferences Manager used with SCCM, PolicyPak
Cloud, or an MDM service,
:::


The basic way to use PolicyPak Preferences Manager is to perform the following
procedures:
...
- Deliver the directive to the client machines. If you use Group Policy as the delivery mechanism,
  the directive is received via client machines. You can also use PolicyPak
...
PolicyPak Preferences Manager has the following main components:

- A management station — Start out by creating a standard Group Policy Preference item using the
  GPMC you use every day. Then export the settings using the PolicyPak Preferences
  Manager Export wizard. You can export these settings for use with PolicyPak Cloud,
...
- The PolicyPak client-side extension (CSE) — This runs on the client (target)
  ...
  separate to install.
- Endpoints — Endpoints must be licensed for PolicyPak Preferences Manager using one
  of the licensing methods.
- PolicyPak Exporter (optional) — A free utility that lets you take Microsoft Group
  Policy Preferences and our other PolicyPak XML data files and wrap them into a
  portable MSI file for deployment using Microsoft Endpoint Manager (SCCM and Intune), or your own
  systems management software.

![about_policypak_gpo_export_1](/images/policypak/preferences/about_endpointpolicymanager_gpo_export_1.webp)

PolicyPak Preferences Manager does not require any particular type of domain
...
(Windows Server 2008 or higher) must have the PolicyPak CSE installed and licensed.
