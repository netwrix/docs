...
:::warning
Reboot the machine or service after enabling this setting.
:::
...
- Optical Character Recognition - if enabled, Endpoint Protector can inspect JPEG, PNG, GIF, BMP, and TIFF file types for content. This option will also change the global MIME Type Allowlists.
...
4. Panic – This mode should be selected under extreme situations when a user’s malicious intent or
   ...
   Protection; all other client modes, except Silent mode, are specific to Device Control.
    ...
    :::info
    Use this mode for selected users/groups/computers only, as it blocks all devices and generates a high volume of logs.
    :::
...
- Deep Packet Inspection - if enabled, Endpoint Protector can inspect network and browser traffic for content. Both the Deep Packet Inspection Allowlists and URL and Domain Denylist require this option
...
- Peer Certificate Validation – enable this setting to turn on the Endpoint Protector certificate
  validation of the websites that are accessed by the user when DPI is active
    ...
    :::warning
    Disabling ‘Peer Certificate Validation’ will not impact Endpoint Protector functionality. Disable it only when an alternative network traffic inspection product, such as a Secure Web Gateway Solution, is validating website certificates.
    :::
...
- Block Unsecured Connection - if enabled, Endpoint Protector blocks unsecured HTTP access and restricts user access.
...
- DPI Bypass Traffic – this setting automatically bypasses non-inspectable traffic and sends an event
  for allowed traffic.
    ...
    1. Bypass DPI Certificate Rejection by Third-Party Applications
        ...
        - Enable this setting if source applications, such as web browsers, encounter SSL errors like:
...
:::note
The Intercept VPN Traffic feature is only available when the Deep Packet Inspection feature is enabled. It will only work for macOS from version 11.0 onwards and only if you also add the Deep Packet Inspection Certificate.
:::
...
To maintain a streamlined process, Endpoint Protector enforces a timeout period of two weeks. During this time frame, Endpoint Protector retains the state for bypassed websites. Beyond this period, Endpoint Protector automatically removes the bypass state, contributing to effective resource management.
...
Before a "DPI certificate rejected" event, use Wireshark to diagnose network traffic. The presence of a "TLS alert" error in Wireshark signals the impending event.
...
- File Tracing – use this feature to monitor data traffic between protected endpoints and
  removable devices, internal eSATA HDDs, and Network Shares. It also shows other actions that occurred, such as files named, deleted, accessed, modified, etc.
...
:::note
For your deployment, activate File Shadowing for not more than 15% of your total endpoint capacity (e.g., for a 1000-endpoint deployment, set File Shadowing to a maximum of 150 endpoints for optimal performance). For more users, contact customer support for recommended settings.
:::
...
extension ID configured in the GPO is updated. Endpoint Protector will soon deprecate the old extensions. Customers
...
completing the Endpoint Protector Client upgrade process, you must perform a full computer reboot for the
...
meets their organization's requirements. The default maximum file size is 40 MB, with a maximum limit of 4096 MB.

Furthermore, users have the flexibility to configure additional file type sizes, which default to the following values: PDF (2048 MB) and Archives (256 MB). You can adjust these file type sizes within the range of 1 KB to 4 GB to accommodate your specific needs.

Additionally, in the Windows environment, Windows applies a default time-out of 10 seconds. For MacOS, macOS enforces a strict 10-second time-out due to Apple OS architecture, which terminates processes that don't respond promptly. Linux operates without a specific time-out limitation.
