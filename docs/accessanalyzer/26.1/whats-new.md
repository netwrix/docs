...
One installer binary, `dspm-installer`, sets up the whole product on a single Linux server. You pick a size (small, medium, large, or enterprise) and supply a TLS certificate for the server's hostname. The installer prints a temporary password for the first administrator, who changes it at first sign-in and can connect Active Directory or Entra ID then or later. See [Installation](install/index.md).
...
An agent is a Linux machine that runs scans. The System agent on the Access Analyzer server is there from the start. To reach a segmented network or keep scan traffic local, deploy more agents over SSH and route scans to them with labels. See [Agents](agents/index.md).

## Scans

A scan says what to collect, from which sources, on which agent, and when. An Access scan inventories shares, folders, files, sites, and their permissions; a Sensitive data scan classifies file content against sensitive data patterns; an Identity sync pulls users, groups, and memberships from a directory. A scan targets specific sources or every source matching a set of labels, and runs on demand or on an hourly, daily, weekly, or monthly schedule. Each run creates one execution per source, which you can pause, resume, or stop; completed executions stay in the history for 90 days. See [Scans](scans/index.md).

## Sensitive data patterns

A sensitive data pattern is a regular expression with a confidence level of Low, Medium, or High. Access Analyzer ships 139 built-in patterns in 11 groups such as PCI DSS, GDPR, and Credentials, and scans classify by group. Add your own patterns and groups, place custom patterns in built-in groups, and test any pattern against sample text before saving. See [Sensitive data patterns](sensitive-data-patterns/index.md).

## Dashboards and reports

Two dashboards give the wide view: Data security for File Server and SharePoint Online sources, and Active Directory for your domains. Reports answer one question at a time on three pages: Data (11 reports), Identity (three), and Compliance, which arranges eight Data reports under seven frameworks (GDPR, HIPAA, PCI DSS, SOX, ISO 27001, NIST CSF, and SOC 2). Both open in one viewer with filters and drill-down. See [Dashboards and reports](dashboards-reports/index.md).
