...
- Time to Retain Events – Time period to retain events not related to any threats. The job removes
  threats with the "False Positive" status at this interval. By default, this is set to 1 Week. Use
  the dropdown menu to select another interval, which includes 1 Week, 1 Month, 3 Months, and 6
  Months as options.
- Time to Retain Threat Events – Time period to retain events related to any threats. The job still
  retains the primary event for each threat. By default, this is set to 1 Month. Use the
...
- Last Heartbeat – Date timestamp of the last agent connection check
- Next Run Time – Date timestamp for the next time the job will run
