...
given site isn't listed here, the activity login may fail as it isn't an authorized site for
login.

![URLs Tab for Entra ID](/images/privilegesecure/26.03/accessmanagement/admin/policy/page/details/azureaddetailspage.webp)

The URLs tab has the following features:

- Add — Opens the Add Website URL window. See the
[Add Website URL Window](/docs/privilegesecure/26.03/admin/resources/detailspages/website/urlswebsite/addwebsiteurl.md) topic for additional
information.
...
- Regex — A match is made on the valid regex string
- Host — A match is made on the host name (FQDN)
