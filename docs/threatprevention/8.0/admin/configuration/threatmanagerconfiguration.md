...
identifier (URI) and the Threat Manager App Token. This window sends policy event data to Threat
Manager.
...
![Netwrix Threat Manager Configuration window - Event Sink tab](/images/threatprevention/8.0/admin/configuration/threatmanagerconfiguration.webp)
...
**Step 4 –** You can enable SSL for the Advanced Message Queuing Protocol (AMQP) event stream.

As Threat Manager supports TLS/SSL, you can also enable SSL in Threat Prevention to communicate
securely with Threat Manager.

- If Threat Manager has TLS enabled, select the **Enable SSL** checkbox to enable the Agent to send
  events to Threat Manager. Otherwise, the Agent throws an error when connecting.
...
**Step 6 –** The Policies area lists all policies that exist in Threat Prevention. It displays the
following for each policy:

- Send – When you select this checkbox, the policy sends the event data to Threat Manager directly
  from the Agent. You can also set this option using the Send to Netwrix Threat Manager checkbox on
...
Threat Prevention now sends all real-time event data from the selected policies to Threat Manager.
...
On the Honey Token tab, you specify a samAccountName, which Threat Prevention substitutes with the
replacement samAccountName that you provide. Threat Prevention sends the information on this tab to
the Threat Prevention Agent. If the Agent sees an LDAP query using information from the Honey Token
of fake accounts, it alters
...
![Netwrix Threat Manager Configuration Window - Honey Token tab](/images/threatprevention/8.0/admin/configuration/honeytoken.webp)
...
**Step 3 –** Add or remove the RIDs of groups to monitor on the Settings tab. See the
[Forged PAC Analytic Type](/docs/threatprevention/8.0/admin/analytics/forgedpac.md) topic for additional information.
...
**Step 5 –** After you set the configurations, click **Save**. The Configure Analytics window
closes.

**Step 6 –** In Threat Prevention, click **Configuration** > **Netwrix Threat Manager
Configuration** on the menu. The Netwrix Threat Manager Configuration window opens.

![Netwrix Threat Manager Configuration Window - Forged PAC tab](/images/threatprevention/8.0/admin/configuration/forgedpac.webp)
...
When Threat Prevention triggers a Forged PAC analytic, it sends the event data to Threat Manager.
