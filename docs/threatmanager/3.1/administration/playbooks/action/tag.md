...
- Action – Specify whether to add or remove tags. If not specified, the tag is added.
...
- Users – The users to have their RDP Session ended. If not specified, the user who triggered the
  threat is used. Select the users from the dropdown.
- Credential – The domain credential used to run the action. Domain credentials are populated by
  ...
  under the credentials of the action. Select the credentials from the dropdown.
