...
Refer to the [NPS-D API documentation](pathname:///docs/privilegesecurediscovery/2_22/integrations/api/advancedapireference) for a full list of available endpoints and usage. For Postman-specific authentication steps, see [Authenticate Using Postman](pathname:///docs/privilegesecurediscovery/2_22/integrations/api/postmanauthenticate).
...
5. Navigate to the **SecureONE API collection**, expand **API Keys**, and select **Authenticate using an API key**.

   ![SecureONE API collection with Authenticate using an API key selected](../0-images/postman-api-collection.png)

6. Click the **Body** tab and enter the **userId** associated with the API key.
...
8. Click **Save**, or use **Save As** to store it separately.

   ![Save button](../0-images/postman-save.png)

9. Click **Send** and verify the response appears at the bottom. You should receive a token in the response.

   ![Send button and response at the bottom](../0-images/postman-send-response.png)

10. You can now send **GET** and **POST** requests using the stored `current_jwt` token.
