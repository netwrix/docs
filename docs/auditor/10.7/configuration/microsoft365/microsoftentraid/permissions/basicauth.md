...
- To collect activity data without logons, the privileged role can be revoked from the specified
  account after the initial data collection. Ongoing audit data collection uses Microsoft
  APIs with access permissions granted to the Microsoft Entra ID app, therefore requiring no tenant-level
...
Security Reader role is required.
...
For more information on Administrator role permissions, see the Microsoft article
