...
PolicyPak Application Settings Manager has a special mode, called Switched mode, which
...
In the following example, we're creating a Switched policy with PolicyPak Application
...
![policypak_application_settings_1_19](/images/policypak/applicationsettings/modes/endpointpolicymanager_application_settings_1_19.webp)
...
- `ppSwitched_ onXmlData.log`—when directives are delivered via MSI, file, or PolicyPak Cloud service
- `ppSwitched_onSchedule.log`—when directives are redelivered using the PolicyPak
...
:::note
Experienced Group Policy administrators may note the similarities between PolicyPak Application Settings Manager Switched mode and Group Policy's own loopback processing mode.
PolicyPak Application Settings Manager Switched policies can be handier than loopback
processing because with PolicyPak Application Settings Manager Switched mode, you can
