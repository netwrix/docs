...
Two current rules read as disagreements with the style guide's literal wording but are deliberate elaborations, not bugs — do not "fix" them back to the absolute reading without a real decision, the same way the exceptions in §13 and elsewhere are deliberate: **§12's product-name-in-title** sharpens the style guide's absolute "do not include product names in titles" into a general rule with a stated soft-reminder carve-out for genuinely distinguishing component names. **§13's title/H1 row** clarifies that a body with no H1 is a valid state (Docusaurus renders the frontmatter `title` as the H1 when the body has none), not a violation of the style guide's "the article title is always H1" line, which is about heading level, not literal presence.
...
- **`BoilerplateCrossRef` is unreachable on KB files — do not treat it as a real finding.** The Vale rule lives only in `.vale/styles/Netwrix/`, never `.vale/styles/NetwrixKB/`. `.vale.ini` scopes `BasedOnStyles = NetwrixKB` to `docs/kb/**/*.md`, replacing (not merging with) the repo-wide `Netwrix` style — so a `BoilerplateCrossRef` finding can never fire on a KB file under this scope (confirmed empirically: `vale --config .vale.ini` on a KB test file with a `BoilerplateCrossRef`-triggering phrase produced zero alerts). If one somehow appears, verify the file's actual style scope before treating it as real.
...
- **Title case correction** (e.g., "configure stopwords" → "Configuring Stopwords"). Applies to all article types. **When the body has an H1 *and* the skill running this rulebook area also runs Vale in the same report (e.g., `kb-pr-open`, `kb-pr-review`), defer to Vale — do not duplicate the finding.** `NetwrixKB.HeadingCase` (`scope: heading`) already fires on the H1 in that case, so it's Vale's finding, not a second row for the same title; do not flag the same H1 twice. A standalone invocation that doesn't run Vale (`/derek` on its own) has no deferral to make — Derek's own "what Vale already handles" list already assigns heading case to Vale, so a title-case issue on an H1-bearing article goes unchecked in a `/derek`-only run (nobody runs `NetwrixKB.HeadingCase` in that invocation), same as before this rulebook existed. **When the body has no H1 at all (§13's title row), the deferral does not apply regardless of which skill is running — check the frontmatter `title` value's case directly,** since Vale's heading-scoped rule never evaluates a frontmatter field and nothing else will catch a badly-cased title on those articles.
...
| `products` | Present, contains at least one product ID from the canonical list in `src/config/products.js` — no hyphens or underscores as word separators (`dataclassification`, `directorymanager`, `threatmanager` are correct; `data-classification`, `directory-manager`, `threat-manager` are not). `src/config/products.js` is authoritative if it and `kb_style_guide.md`'s Product Names table disagree — the style guide table can lag a `products.js` update. |
| `tags` | Present and includes `kb` |
| `knowledge_article_id` | See policy below |

**Product ID carve-outs.** Three values are exceptions to the strict-match rule:

- `onesecure` (5 files) has a real target — flag and correct to `1secure`, the actual `products.js` ID. This is a normal typo/legacy-value correction, not a carve-out.
- `general` (44 files under `docs/kb/general/`) has no valid `products.js` target — cross-product content with no corresponding product. Never flag.
- `recoveryad` (1 file) has no single valid target — `scripts/copy-kb-to-versions.mjs` maps both `recoveryforactivedirectory` and `identityrecovery` to this KB folder, so there are two equally defensible IDs and no way to pick one mechanically. Never flag; ask which product the article is actually about instead of guessing.
