...
You must generate an API Access Key for Nasuni activity monitoring. The Nasuni Edge
Appliance generates its own audit trail. The Activity Monitor uses an API Access Key to form a
...
The Activity Monitor requires both the Key Name and the Key Passcode to connect to
