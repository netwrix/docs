...
information. You can use one set of credentials to log in to many different
websites. Managing one login per user is simpler than maintaining separate logins for email, Customer
...
For users to be able to use SAML, Simple Mail Transfer Protocol (SMTP) must be set up and an email address must be stored with the
respective users.
