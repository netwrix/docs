...
- Policy Refresh Interval (sec) – enter the time interval at which the Client checks with the Server
  and updates with the latest settings, rights, and policies.
    ...
    :::note
    Azure Active Directory sync intervals (or Active Directory syncs) may affect the policy refresh cycles if Endpoint Protector is conﬁgured to sync entities. Consider
...
- File Tracing Direction – this setting lets you monitor ﬁle transfers based on transfer
  direction:
...
1. Google Chrome
    ...
    **Step 3 –** Conﬁgure your Group Policy as shown in the following screenshot.
