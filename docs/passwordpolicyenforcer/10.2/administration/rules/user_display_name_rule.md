...
name (full name for local accounts). Passwords that are similar to a user's display name aren't
...
- Choose a value from the **Tolerance** dropdown list to specify the maximum number of consecutive
  ...
  consecutive matching characters (shown in bold type). Password Policy Enforcer rejects this
  password if the tolerance is four or lower, and accepts it if the tolerance is five or higher.
