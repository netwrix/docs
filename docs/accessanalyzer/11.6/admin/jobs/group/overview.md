...
Job groups manage related jobs and can contain sub-job groups to ensure that related
...
jobs must run before the analysis and reporting jobs that rely on the collected data without
...
functions you can access in the Jobs Tree in the Navigation Pane.
...
:::note
If applicable, the page shows special instructions for which hosts to target for
...
Job group settings can be applied directly or inherited. At the job group level, all settings
apply directly.
