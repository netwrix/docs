...
- Type – Select a protocol type from the dropdown list to use when sending a Syslog message to a
  SIEM application:
    - UDP
    - TCP
...
- Template – Select the template you want from the dropdown list
    ...
    - LEEF template – Threat data is sent to the SIEM application in the standard Log Event Extended Format (LEEF) format
    - CEF template – Threat data is sent to the SIEM application in the standard Common Event Format (CEF) format
