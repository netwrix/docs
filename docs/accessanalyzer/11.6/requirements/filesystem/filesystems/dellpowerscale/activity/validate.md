...
configure the Dell Common Event Enabler (CEE) automatically if it is installed on the same server as
the agent. This needs
...
:::note
Use debug logs only for troubleshooting. Disable them by default.
:::
