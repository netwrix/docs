...
| **article-type: heading-labels** | **Article Types** | **Resolution section heading pluralization:** Section headings in Resolution articles — both Error and Symptom formats — are always singular — `## Symptom`, `## Cause`, `## Resolution` — even when the section contains multiple items. Flag `## Symptoms`, `## Causes`, `## Resolutions` as Required fixes. |
...
| **product-names** | **Product Names** | Always cross-check product IDs against `src/config/products.js` — the style guide table may be outdated. The config file is authoritative. **Exceptions:** `onesecure` has a real target (`1secure`) and should be flagged and corrected. `general` (cross-product content, no corresponding product) and `recoveryad` (ambiguous — `scripts/copy-kb-to-versions.mjs` maps both `recoveryforactivedirectory` and `identityrecovery` to this KB folder, so there's no single correct canonical ID to propose) have no valid `products.js` target at all — do not flag these as Required fixes. |
...
| **images: location** | **Screenshots** | KB image structure is not in the style guide — apply this rule: images must be stored in `0-images/` at the **product level** (`docs/kb/<product>/0-images/`), not inside category subfolders. PNG, WebP, JPG, and JPEG are all in active use and all valid — do not flag file format. Articles in category subfolders reference them with `../0-images/filename.<ext>`. |
