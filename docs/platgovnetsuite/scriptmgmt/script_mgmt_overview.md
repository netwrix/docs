...
:::note
Platform Governance for NetSuite doesn't document or analyze locked scripts.
:::
...
3. Identify errors that aren't material to your business and mark them as resolved
...
- How often scripts are executing so you can focus on high-volume scripts or identity scripts
...
1. **Locked Script Records**: A locked script record doesn't impact script analysis since Platform
   ...
   performance metrics.
2. **Locked but readable (not hidden) Script File**: Script tags can't be added to locked files,
...
The focus of script analysis can vary based on your business needs. The following guidance is based
...
scripts to processes, letting you apply audit tags and change log levels for scripts
associated with specific processes using the mass update filters.

**How particular scripts or bundles perform**
...
ways to use these tools depend on your objectives and account setup.
...
provide data when the scripts are in Audit or Debug mode. Switch the scripts to
one of those two levels if appropriate.The Add Audit Tags mass update backs up and then adds
start tags to all of your unlocked scripts. This tracks execution frequency. You can
