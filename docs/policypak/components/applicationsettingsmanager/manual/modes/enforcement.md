...
When actions are set for an element (using PolicyPak DesignStudio), enforcement modes
become available after right-clicking on them in the PolicyPak Application Settings
...
- "Always reapply this setting." This is the default. This setting ensures that if a user changes
  ...
  value, or selects a different radio button), PolicyPak Application Settings Manager
...
GPO falls out of scope. Users are then free to change the setting, and PolicyPak will
