...
reports. Use them to view consolidated recent event activity for EPE or LDAP which spans all EPE or
LDAP policies. The reports include default groupings that best show the consolidated data.
...
available, the grid doesn't populate but the columns display on the grid. The following buttons are
...
- Export – Export the selected report to an Excel, PDF, HTML, RTF, or CSV output. The Save As window
  displays when you select an export option. Provide a name for the report and save to a specified
  location.
...
The Parameters window displays the following options. Select the options you want and click **OK** to
...
- Range From/To – If left unchecked, the report displays all policy data. When enabled, set a data
  range for the data to display in the report using the To and From dropdown calendars. By default,
  the calendar displays today's date. Click **OK** to set the date and time.

  For faster navigation on the calendar, select the month to display all twelve months to choose
  from. After the months display, select the year to display a range of years to choose from.

- Policies – Choose **All** or **Enabled Only** to display all policies or enabled only policies in
  the report
- Events – Select **All**, **Blocking**, and/or **Monitoring** events to display in the report
