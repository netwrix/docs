...
identifier and the Threat Manager App Token. Threat Prevention sends policy event data to Threat
Manager through this window.
...
![Netwrix Threat Manager Configuration window - Event Sink tab](/images/threatprevention/8.1/admin/configuration/threatmanagerconfiguration.webp)
...
Because Threat Manager supports TLS/SSL, you can enable SSL in Threat Prevention to communicate with
Threat Manager securely.

- If Threat Manager has TLS enabled, then select the **Enable SSL** checkbox to enable the Agent to
  send events to Threat Manager. Otherwise, the Agent throws an error when connecting.
- Select the **Ignore Certificate Errors** checkbox to ignore certificate errors that may arise when
  the Agent connects to Threat Manager. If this checkbox isn't selected, the Agent fails to
...
Threat Prevention now sends all real-time event data from the selected policies to Threat
Manager.
...
To send the Honey Token to Threat Manager for an LDAP Deception trap:
...
![Netwrix Threat Manager Configuration Window - Honey Tokem tab](/images/threatprevention/8.1/admin/configuration/honeytoken.webp)
...
To include the Forged PAC information in events:
...
![Netwrix Threat Manager Configuration Window - Forged PAC tab](/images/threatprevention/8.1/admin/configuration/forgedpac.webp)
