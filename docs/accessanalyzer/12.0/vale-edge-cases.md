...
| Export tool | The system provides the ability to export data, e.g. CSV files | Active |
...
| Utility | Utilize the built-in tool to configure etc. | Active |
...
|---------|---------|-------|
| Timeout | 30s | Make sure the timeout does not exceed the maximum, i.e. 60 seconds |
...
The text after the code block should still be fixed: make sure to setup the drop-down.
