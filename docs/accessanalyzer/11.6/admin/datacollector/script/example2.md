...
external to Enterprise Auditor with only one thread.
:::


In this example, the script opens a connection with a SQL server. Enterprise Auditor provides the
SQL server name during the query and provides the active host to the script using
...
construct a query to the File System Data Collector, which ultimately returns the size of the file.
