...
To avoid having too much duplicated information on each line in a CSV file, define a primary key for the file which will allow the pooling of common information. Concatenate the values of the columns `Login` and `Company ` with a `-` as separator in an `Id` column, which will be defined as key for the file.
...
This file is automatically generated by a script and the suffix (`123`here) is incremented on each generation. Thus, use a regex to avoid changing the command line for each new generated file.

`--input-file C:/identitymanagerContoso/Sources/hr_example(.*?).csv --regex --separator ;`

The file contains two headers with the same name, each related to one kind of group. Thus, rename one of these headers.

`--input-file C:/identitymanagerContoso/Sources/hr_example(.*?).csv --regex --separator ; --headers-edit-name "Members MembersAzure"`

In this example, the two Members columns contain all members for each group separated by a `,` for the first Members column, and by a `*` for the second one. Transform these columns in Identity Manager's format for multi-valued attributes.
...
## Arguments
 ...
 | --headers-remove-index optional | **Type** Integer **Description** Specifies the headers to remove by index. Use this command to remove the second occurrence of a duplicate header by specifying its index. **Example** Remove header located at index 5: `--headers-remove-index 5`. | 
 ...
 | --columns-key optional | **Type** String List **Description** Specifies the columns key to delete duplicates (the first line found is kept). A column created by this tool can be specified as a key column through this argument, like the columns created by the `--columns-concat` for example. **Example** Define columns `RawId` and `ID` as keys: `--columns-key RawId ID`. | 
 | --- | --- | 
 | --- | --- | 
