...
- Token Username – The username used by the Honeytoken deployment process and threat
  detection. This is a static username used for all deployments for this policy.
    ...
    username are no longer detected.
    :::
...
- Token Password – Specify a password string to be used for the Honeytoken deployment. Passwords
  ...
  random numbers.
    ...
    :::note
    If the Honeytoken script can't reach the Threat Manager URL, the Honeytoken is
    still deployed but Threat Manager is unaware of the Honeytoken deployment status.
    :::
...
- Select Credential Profile – Lists Credential Profiles added on the
  [Integrations Interface](/docs/threatmanager/3.1/administration/configuration/integrations/overview.md). Select a Credential Profile from the
  dropdown.
...
- Token Time to Reset Password – All Honeytokens for a policy share a common password. This value
  specifies how frequently a new password is generated for the Honeytoken policy. Select a
  value using the dropdown from the following options:
    - 1 Day
    - 7 Days
    - 14 Days
    - 30 Days
...
- Test Policy Button– Click the Test Policy button to run an LDAP query for the token username
  ...
  and sends to Threat Manager. If Threat Manager receives the event within three minutes, the
...
- Copy LDAP Filter Button– Clicking the Copy LDAP Filter button automatically copies to the
  clipboard the exact string required for Activity Monitor or Threat Prevention to configure
...
Click Save to update the policy settings. After saving, Threat Manager detects threats for this Honeytoken user.
...
Action Service to deploy Honeytokens to remote computers, or a PowerShell script can be
...
- Host Deployment:
    ...
    - Schedule – Select a schedule to automatically deploy Honeytokens. Once
      a schedule is designated, deployment is queued immediately and then follows the
...
When the Deploy Now button is clicked, Threat Manager immediately deploys the Honeytoken to all
...
Closing this window won't cancel the deployment.
