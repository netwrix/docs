...
The most common problem with PolicyPak Remote Work Delivery Manager is that files do
not copy as expected. Here are some tips when trying to troubleshoot PolicyPak Remote
Work Delivery Manager.

The log files for PolicyPak Remote Work Delivery Manager are found in the following
folder: `%Programdata%\PolicyPak\PolicyPa`k Remote Work Delivery Manager. This is because PolicyPak Remote Work Delivery Manager affects the Computer side (and all users on that
...
because all PolicyPak Remote Work Delivery Manager work happens on the Computer side.
...
- `ppUser_onPolicyChanged.log`: New data is added to this log when Group Policy applies in the
  background or when a non-Group Policy method is used (MEMCM, PolicyPak Cloud, and so
  on).
...
Figure 55 is an example of a PolicyPak Remote Work Delivery Manager log with some
annotations.

![tips_security_and_troubleshooting_6](/images/policypak/troubleshooting/remoteworkdelivery/tips_security_and_troubleshooting_6.webp)

Figure 55. An example of a PolicyPak Remote Work Delivery Manager log.

Then, to see details of what PolicyPak Remote Work Delivery Manager is trying to do,
you can open up the PP_Operational.log. There will be two PP_Operational logs for PolicyPak Remote Work Delivery Manager (see Figure 56):
...
![tips_security_and_troubleshooting_7](/images/policypak/troubleshooting/remoteworkdelivery/tips_security_and_troubleshooting_7.webp)
