...
The .Active Directory Inventory Solution is configured to run with the
default settings, but some settings are optional for configuration. It can be run directly or
scheduled.
...
Run the jobs in the .Active Directory Inventory Job Group together and in order by running the
...
**Step 3 –** Schedule the .Active Directory Inventory Job Group to run as desired.
