...
**Step 2 –** Relaunch WinZip and reopen Options | Configuration. You will see that PolicyPak Application Settings Manager will redeploy the settings automatically. PolicyPak Application Settings Manager will reapply changes if the client is online (with network
connectivity) or offline (disconnected from the network).

PolicyPak Application Settings Manager will silently, automatically reapply
application changes when the application is run. This is set automatically for all applications
under PolicyPak Application Settings Manager by default, but can be changed. For more
information on PolicyPak Application Settings Manager's automatic reapplication of
