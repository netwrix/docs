...
![settings](/images/activitymonitor/9.0/config/dellpowerscale/settings.webp)
...
need monitoring isn't done through the Activity Monitor console. Configure OneFS event monitoring
...
![eventforwarding](/images/activitymonitor/9.0/config/dellpowerscale/eventforwarding.webp)
...
:::info
When deploying multiple Dell CEE instances at scale, configure an
accommodating agent with each CEE instance. If multiple CEE instances send events
...
Next, configure the monitoring agent on the Windows server to monitor the
