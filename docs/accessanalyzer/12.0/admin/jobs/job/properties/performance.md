...
The Performance tab provides options you can use to improve job performance and runtime.
...
- Skip Hosts that don't respond to PING – Selected by default. Deselect if a target host has been
  configured to not respond to PING requests, allowing Access Analyzer to scan the target host
  without a PING response.

    :::note
    In most cases, it isn't recommend to deselect this option, as it causes the job to
