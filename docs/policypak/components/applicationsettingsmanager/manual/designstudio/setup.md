...
![setting_up_application_configuration](/images/policypak/applicationsettings/designstudio/configurationdata/setting_up_application_configuration.webp)
...
![setting_up_application_configuration_1](/images/policypak/applicationsettings/designstudio/configurationdata/setting_up_application_configuration_1.webp)
...
- Mozilla-specific config file: The Mozilla Corporation has a variety of applications which all work
  ...
  Corporation all use this file type. There's no need to use Netwrix PolicyPak DesignStudio to create your own Firefox or Thunderbird AppSets; we've already
...
:::note
PolicyPak DesignStudio will create new data files for all file types if they
don't already exist on the client computer. This is true for every file type PolicyPak
:::


DesignStudio supports, except RDP files. RDP files are not created on the client machine, and they
must already be present to be modified by PolicyPak

DesignStudio. Additionally, it is now recommended that if you wish to deliver and maintain `.rdp`
files, you do so with PolicyPak Remote Desktop Protocol Manager
