...
The service address of the SQL server can be selected via the dropdown menu. It is mandatory to
...
If the option “Service user” is selected, enter the user that logs on to the SQL Server. 
...
:::note
Refer to the system requirements for server section for details on required user permissions.
:::
...
:::note
If this option is changed, the server needs to be restarted so that the change can take
effect.
:::
