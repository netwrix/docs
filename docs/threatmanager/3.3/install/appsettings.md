...
when no override is present. This file is replaced on every upgrade and shouldn't be edited
...
file isn't created by the installer — you must create it yourself. Settings in `appsettings.json`
...
Changes to `appsettings.json` don't take effect until the service is restarted. To restart a
