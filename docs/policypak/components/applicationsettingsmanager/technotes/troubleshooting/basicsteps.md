...
Netwrix PolicyPak Application Manager is a relatively simple
...
emailing PolicyPak technical support.

The most common tech support question we get is "Why aren't PolicyPak Application
Manager settings applying to my client machines?" Here are some items to check.

YES. REALLY CHECK EACH ONE, one by one please.

**Step 1 –** Did you really go through the PolicyPak Application Manager Quickstart
...
**Step 3 –** Did you create the PolicyPak Application Manager settings within the GPO
...
both Firefox and IE Paks in the MANUALS section of the PolicyPak Portal.
...
PolicyPak Application Manager to work properly.Run GPresult /R …AS THE LOCAL ADMIN
...
- Alternatively, try renaming the computer to COMPUTER1 (or similar) such that COMPUTER is in the
  name. When you do this, the PolicyPak CSE will act as if it's fully licensed. If
  PolicyPak Application Manager starts to work – you have a licensing issue.
...
- For instance, as a test, in the same GPO where you're deploying PolicyPak settings,
...
- If you are correctly prevented from accessing the Control Panel, it could be a PolicyPak Application Manager issue, as you are clearly getting Group Policy delivered correctly. If
  you are not restricted from the Control Panel, you aren't getting Group Policy correctly, so
  PolicyPak Application Manager cannot function.
...
**Step 11 –** Are the right Registry values present in the PolicyPak Application
Manager project? Reopen the source pXML (XML) file using PolicyPak DesignStudio. Check
...
If \*AFTER\* these steps you are not getting results from PolicyPak application
manager…
