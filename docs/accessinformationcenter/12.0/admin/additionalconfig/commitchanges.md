...
Configure the Access Information Center to commit changes to Active Directory (AD) group
...
When you manage File System or SharePoint resources through the Access Information Center (AIC), configure access groups for those resources in the target environment. An access group provides one
...
**Step 1 –** Create a domain user which isn't a member of any group other than Domain Users to be
...
- The **Use the account running this service: [domain]\[username]** option isn't a least privilege
...
If access groups assigned for resource management through the Access Information Center don't
