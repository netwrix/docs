...
- Choose a value from the **Tolerance** dropdown list to specify the maximum number of consecutive
  ...
  characters (shown in bold type). Password Policy Enforcer rejects the new password if the
  tolerance is five or lower, and accepts it if the tolerance is six or higher. Choose the
...
only enforced if the Password Policy Client is installed. It doesn't store or transmit passwords or
password hashes.
