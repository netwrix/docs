...
Sensitive Data Discovery Auditing scans on SharePoint farms. You can configure the Netwrix Activity
Monitor to monitor activity on SharePoint farms and make the event data available for Access
...
See the [SharePoint required permissions for Access Analyzer](/docs/accessanalyzer/12.0/requirements/sharepoint/sharepoint/sharepoint/access.md) topic for
instructions.
...
certain inbound rules. These scans operate over a default port range, which can't be specified via
an inbound rule. For more information, see the Microsoft
