...
Nginx offers several [load balancing methods](https://docs.nginx.com/nginx/admin-guide/load-balancer/http-load-balancer/#load-balancing-methods) which are all compatible with Identity Manager.
...
All Identity Manager servers also share a database. See the [Application Settings](../integration-guide/network-configuration/agent-configuration/appsettings) topic for additional information.
...
should be set to `80`, except if you have another web server listening for port 80 requests and passing them to your nginx server.
...
Load balancing involves at least two Identity Manager servers to which [nginx, acting as a load balancer](https://docs.nginx.com/nginx/admin-guide/load-balancer/http-load-balancer/), distributes the load of incoming requests.
...
The following configuration defines a group named `<b>usercubegroup</b>` which contains two server configurations, each one resolving to an actual Identity Manager's server instance:

```text
...
http {
    upstream <b>usercubegroup</b> {
