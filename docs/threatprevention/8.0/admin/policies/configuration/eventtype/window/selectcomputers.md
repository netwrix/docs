---
title: "Select Computer Window"
description: "Select Computer Window"
sidebar_position: 100
---

# Select Computer Window

The Select Computer window provides a list of available computers. Selected objects are added to the
filter from where you clicked the **Add** (+) button to open this window.

![Select Computer Window](/images/threatprevention/8.0/admin/policies/eventtype/window/selectcomputers.webp)

- Select either Browse Mode or Search Mode.

- Select an Agent from the dropdown menu and click **Connect**.
- Navigate through the domain tree and check the items you want in the **Results** pane on the
  right.
- Then click **OK** to close the window.

See the [Selection Windows](/docs/threatprevention/8.0/admin/policies/configuration/eventtype/window/overview.md) topic for additional information.

The selection is displayed in the appropriate box of the Hosts (from) filter or the Hosts (to)
filter.

:::note
If the selected Agent isn't configured to "Enable DNS Host Name Resolution," then the
Results pane may not include the DNS name or IP address for computer identification purposes. See
the [DNS Host Name Resolution ](/docs/threatprevention/8.0/admin/agents/deploy/setoptions.md#dns-host-name-resolution)topic
for additional information.

:::
