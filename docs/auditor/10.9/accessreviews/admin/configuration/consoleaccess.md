...
**CAUTION:** Before disabling the Builtin Administrator account, you must first assign at
...
[URL & Login](/docs/auditor/10.9/accessreviews/admin/login.md) topic for information on how users will log in and where they are
...
- Domain — If the Access Reviews Console has been configured for multiple domains, use the dropdown
  menu to select the domain you want
...
- Access is enabled – A user's account must be enabled to log into the console. Uncheck this
  option to grant access at a future time.
...
After you add the first user with the Administrator role, that user can disable the Builtin
Administrator account. See the Modify the Builtin Administrator Account topic for additional
information.
...
- Access is enabled — Indicates whether you can use the account to log in
- Change Password — Use this option to change the password for this Builtin Administrator account. Check
