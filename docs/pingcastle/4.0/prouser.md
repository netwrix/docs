...
rule for a period that you specify. Use this only for very specific
reasons, such as when a rule doesn't apply in
...
PingCastle Pro allows administrators to consult all report data and make decisions. You can't yet
launch scans directly from the application, so you must launch the PingCastle tool in standalone
