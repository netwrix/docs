...
3. Grant full access to the `/ifs/.ifsvar/audit/` catalog for `BUILTIN\Administrators`:
...
Starting from Dell Isilon/PowerScale 9.2.0 and above, the `lwnet` command is considered deprecated. When configuring audit manually, you see the following warning:
...
This command is required to create a shared folder pointed to `/ifs/.ifsvar/audit/`. Ignore the warning.

5. Add the `BUILTIN\Administrators` group in the share permissions for the `netwrix_audit$` folder with `full access` rights:
...
:::note
Verify the correctness of the path to the Isilon audit log files in **Monitoring Plans** > `<MP name>` > `<Isilon item name>` > **Edit item** > **General** > **File share UNC path to audit logs**.
...
6. Enable protocol auditing for a selected zone (for example, `zone1`). Do one of the following, depending on your Dell Isilon/PowerScale storage version:
...
7. Create the `netwrix_audit` role in your access zone (for example, `zone1`) and add the required privileges to this role:
