...
2. **Inheriting permissions for records**:
   ...
   is defined as a system standard. This means that there's no difference between the permissions
   for an organisational structure and the permissions for data that's stored in these
   organisational structures.
...
structures can be defined via the ribbon or also the context menu that's accessed using the right
...
the organisational unit itself but rather to data that will be created within it. In general, it's
