...
Log files for PolicyPak Java Rules Manager are found in two places:
...
PolicyPak Java Rules manager logs are contained within the PolicyPak
Java Rules Manager folder.

![troubleshooting_policypak_6](/images/policypak/troubleshooting/javaenterpriserules/troubleshooting_endpointpolicymanager_6.webp)

Figure 35. The PolicyPak Java Rules Manager logs are contained withinPolicyPak Java
...
- `ppComputer_onPolicyChanged` is used when Group Policy, PolicyPak Cloud, and/or
...
- `ppComputer_OnManual` is used when PolicyPak's command ppupdate.exe is run.
- `ppComputer_onSchedule` is used when PolicyPak's internal processes attempt to look
...
where the client-side extension (CSE) is installed. Since the main logs for PolicyPak
