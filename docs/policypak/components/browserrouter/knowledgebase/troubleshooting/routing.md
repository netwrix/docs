...
Netwrix PolicyPak was contacted by Google about the PolicyPak Browser Router Chrome Extension. This is NOT a security concern. They have a policy
...
So what's the problem exactly?: When you install the PolicyPak CSE and use the
PolicyPak Browser Router Chrome
...
In the short term, nothing. PolicyPak Browser Router will continue to operate on the
...
1. If you manually delete the older Chrome extension, then the older PolicyPak Browser
   Router CSE will not be able to re-download the old Chrome Extension.
2. If you delete a user's profile and then re-log on, then the older PolicyPak Browser
   Router CSE will not be able to re-download the old Chrome Extension.
3. If you bring up a brand new machine and then users log on, then the older PolicyPak
   Browser Router CSE will not be able to re-download the old Chrome Extension.

How would I know if I'm affected by PolicyPak being forced to take down some older
PolicyPak Browser Router Chrome Extensions? We have the list of extensions which ARE
