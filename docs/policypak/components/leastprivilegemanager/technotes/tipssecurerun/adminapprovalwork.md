...
applications** is disabled,then the following determines the decision on whether to show the Admin
Approval dialog:
...
- If an untrusted principal owns the file and elevation isn't required, then the Admin
  Approval dialog does not appear, and SecureRun blocks it.
