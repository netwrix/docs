...
The Change Tracker Hub API Reference documents every REST API endpoint that the Change Tracker Hub (the central management server) exposes. This reference comes from the Change Tracker Hub's OpenAPI 3.0 specification, which reflects the API for version 8.2.
...
See [Authentication](/docs/changetracker/8.2/api/authentication.md) for a PowerShell example that authenticates to the Hub and stores the session for subsequent calls.

## Use the interactive explorer

Each endpoint page includes a built-in API explorer. To use it:

1. Obtain a session token by calling `POST /auth/credentials`. See [Authentication](/docs/changetracker/8.2/api/authentication.md).
...
[Download the OpenAPI spec](/openapi/changetracker-hub-8.2.yaml)
