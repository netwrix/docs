...
As expected, some data from your organization is stored within Netwrix PolicyPak Cloud after it is joined by a computer joins. Below is a list of what is stored
within PolicyPak Cloud.

- PolicyPak UUID: This is a random, unique ID generated in the cloud when a computers
...
stored within PolicyPak Cloud.
:::


All communication to and from the client machines with PolicyPak Cloud is always
encrypted. Below is a description of how the client attempts to communicate with PolicyPak Cloud.

- The PolicyPak Cloud client first tries HTTPS (secure HTTP) using port 443 and an
  encrypted PolicyPak Cloud certificate that the client received at the time it
  joined.
- If that is unsuccessful, then the PolicyPak Cloud client tries HTTP using port 80,
  ...
  PolicyPak Cloud client verifies the identity of the server using a hard-coded
  certificate.

:::note
PolicyPak Cloud will usually work using proxy servers with either HTTP or
