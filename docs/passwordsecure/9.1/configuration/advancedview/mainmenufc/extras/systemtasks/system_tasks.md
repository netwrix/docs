...
- **Reports:** Automatically creates a report that's issued in the notifications. This requires a
...
- **Active Directory synchronization:** The comparison with Active Directory can also be automated
  via system tasks. This requires an active directory profile to be created in advance. Only the Master Key profile can be automatically compared.

## Creating system tasks

System tasks can be initiated as usual via the ribbon or also the context menu that's accessed
...
- **Status:** The system task is normally activated and then starts immediately after it has been
  saved according to the defined intervals. If the system task is deactivated here, it's still
...
- **Interval:** The interval at which the system task should be executed is defined here. All
  increments between every minute and once only are possible. It's also possible to enter an end
  date.
