...
1. Specify migration source and folder:
    ...
    - If you created several migration destinations, select one from the "Where
      should the document be migrated to?" list.
...
2. Configure migration options:
    ...
    | Redact the document                                                           | If update of the source item is supported by the source system, then using this option will instruct the program to apply the redaction plan to the source document after its successful migration. **NOTE:** This option isn't available when performing a move (deleting the original item). |
