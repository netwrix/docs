...
with the Control Database permission can perform activity scans due to the function
AzureSQL uses to return the required audit logs. See the
...
:::info
Netwrix recommends creating a new user when using a least privilege access
model to access the AzureSQL database, because the user must exist in the master database and all
target databases. A least privilege access model uses the bare minimum privileges
...
- Control permission on target databases
    ...
    :::note
    You must grant Control permission on any database you want to collect data for.
    :::


Configure the least privilege access model for AzureSQL collections.
...
These steps grant the user Control permission based on the least privilege access model.
