---
title: "VirtualStore Tab"
description: "VirtualStore Tab"
sidebar_position: 30
---

# VirtualStore Tab

The VirtualStore tab has one setting, as shown In the figure shown. This setting is automatically checked
on and is used when applications running as standard users try to write to locations that are not
allowed. This setting was discussed in the section called "Configuration Data in VirtualStore."

![using_designstudio_tools_4_624x174](/images/policypak/applicationsettings/designstudio/tools/options/using_designstudio_tools_4_624x174.webp)

The figure shown. The VirtualStore tab.



