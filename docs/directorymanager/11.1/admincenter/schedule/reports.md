...
6. The Name Preview box displays the schedule name prefixed with \_ReportPortal\_\_; email notifications show the schedule with this name.

7. In the Scheduler Service Name dropdown list, select a Scheduler service that would be
...
8. To add reports to the schedule, click **Add Reports** in the Reports area. The Add
...
schedule uses the settings provided there to generate the report. Moreover, you can't change the
settings here, such as the container and filter settings.
:::


1. In the Object Category dropdown list, select a report category. Available categories are: _All
...
2. The Reports dropdown list shows all reports in the selected category. On selecting a report, one
   of the following happens:

    - The report is displayed in the grid on the dialog box. This is because it has previously been
      added, and you don't need to add it again. If the report has been generated multiple times in
    ...
    - If the report isn't displayed in the grid, you have to add it using the Add button. (The Add
    ...
    first report in the category is auto selected in the **Reports** dropdown list. If that report
...
4. Select the checkbox for a report and click **Add**. The selected reports are displayed in the
...
1. On the Notifications dialog box, enter the email address of recipients to whom you want to send
