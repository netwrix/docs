...
For this first Quickstart test and example, we urge you to use the preconfigured PolicyPak DLL named `pp-WinZip.DLL`, which is supplied in the download from the PolicyPak Customer Portal.
...
`Winzip 14 to 17.XML` is the source file and can be opened and manipulated within the PolicyPak DesignStudio. You won't be using this file right now.
...
![policypak_application_settings](/images/policypak/applicationsettings/preconfigured/quickstart/endpointpolicymanager_application_settings.webp)
...
PolicyPak Admin Console.MSI (32-bit or 64-bit). This must be done on the machine where
