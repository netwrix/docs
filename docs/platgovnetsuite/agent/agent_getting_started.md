...
| Transactional Data Control | Tracks changes to transaction records and events<br />Search types may be Journal entries, Checks, Bills, and so on.<br />This data type can be monitored as a whole (i.e. any changes<br />additions, modifications, deletions), or specified (e.g. only Journal entries that have been modified after being approved, or journal entries entered and approved by the same user, or list of ad-hoc checks only, and so on). |
