...
be added. This happens because the Membership Life Cycle schedule didn't run on the specific days
for temporary membership update. Ensure that the schedule is set to run at a frequency that meets
...
To create a Membership Life Cycle Schedule.
...
:::note
Membership Lifecycle policies aren't applied to OUs specified here. Target OUs and groups are
