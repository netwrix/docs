...
- Default – The default profile applied when a user is assigned multiple authentication profiles.
  ...
  on as desired.
- Authority – The OpenID Connect provider authority URI. Set the Authority path correctly before configuring outgoing redirection. Incorrect settings generally result in a 404 error.
...
- Login Type – The login type to use to log into the account. Use the dropdown to select one
  of the following: Sam Account Name, User Principal Name, Email Address, or Sid
- User Source – The source type to use to validate the user from the token. Use the dropdown
