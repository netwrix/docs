...
The 1-Box_Access Scans Job collects the data which will be further analyzed to provide
...
the Scan Box Permissions Category. If this query isn't configured but has the access token, the query
...
- Scan Query – Collects access data which will be further analyzed to provide details on
  Box access rights, policies, configurations, and content

Before the first execution, authenticate to the targeted Box environment. This
...
The 1-Box_Access Scans Job contains the Scan Query. To configure the query:
...
**Step 7 –** The Authenticate page is where the connection to the target Box environment is
...
:::note
Authentication to the target Box environment only needs to be completed once, before the
