...
The Dell Common Event Enabler (CEE) should be installed on a Windows or a Linux server. The Dell CEE
software isn't a Netwrix product. Dell customers have a support account with Dell to access the download.
...
Install the Dell CEE using the following steps.
...
After you install Dell CEE, complete the
...
one where the Activity Monitor activity agent is deployed. In those cases, manually set the
Dell CEE registry key to forward events.
