...
For ONTAP 9.5 and earlier, use the following commands to create a new firewall policy
...
agent's settings. The agent adds this port to Windows Firewall exclusions automatically. 
ensure the port isn't blocked by other firewalls between ONTAP and the agent.
