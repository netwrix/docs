...
- Time Interval to Run Report Cleanup – The interval to run the Report Cleanup job. At the interval
  specified, Threat Manager removes any reports older than the retention period. By default, this is set
  to 1 Day. Use the dropdown menu to select another interval, which includes 1 Day, 1 Week, and 1
  Month as options.
- Time During the day to run the Cleanup – The time during the day to run the Cleanup script at the configured interval. By default, this is set to midnight, 12:00 AM. Use
...
choose to store certain data types for longer periods of time. Longer periods require larger disk storage space and disk access speeds.
