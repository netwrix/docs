...
description: >-
  ...
  "Email delivery to recipient %recipient% failed." error.
keywords:
  - Event ID 1223
  - Health Log
  - SharePoint
...
```
Email delivery to recipient %recipient% failed.
```
...
- Cause #2 − Verify the `recipient` details in the affected SharePoint monitoring plan.
  ...
  2. In the **Notifications** tab, verify the `recipients`. When adding a `recipient`, click **Send Test Email** to confirm the email was specified correctly.
