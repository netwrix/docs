...
Auditing in Azure Files is disabled by default. Enable auditing for each storage account to be monitored.  
...
It must be a different storage account — it can't be the same account that hosts Azure Files.  

Share such a *logs storage account* among multiple *files storage accounts*.  
...
The *logs storage account* must be in the same Azure region as the monitored Azure Files storage account, but it doesn't need 
