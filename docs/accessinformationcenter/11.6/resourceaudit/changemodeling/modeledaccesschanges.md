...
- Activity – Displays additional information on recent activity the trustee performed that the modeled
  change would have impacted
...
- Impacted – True or False if the modeled change would have impacted recent activity. The activity
  must have occurred within the value set for the **ActivityDays** parameter in the **web.config**
  file. By default, this is set to 90 days. This feature requires the File System Activity Auditing
  (FSAC) collection jobs in Enterprise Auditor to be executed on a regular basis.
...
- Source Path – Location for which the trustee would be granted rights to the selected resource,
  which can be represented two ways:
    ...
    - Directly Applied – The selected trustee receives direct rights
...
The Activity table contains additional information on recent activity the trustee performed that the
modeled change would have impacted. Activity data displays local time stamps. This table will be blank
