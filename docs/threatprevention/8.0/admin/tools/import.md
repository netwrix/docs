...
You may need to export collections, policies, and templates from the Administration Console and import them back for certain reasons, such as during a reinstall. Use the Export Policies and Templates Window to export these items to an XML file. See the
...
Use the Import window to select an already exported XML file for import and configure the import settings.
...
**Step 3 –** In the Destination Folders section, specify a folder from within the folder structure in the Navigation pane for saving the policies and/or templates imported from the XML file. For policies, specify a folder under the Policies node in the navigation pane. For templates, specify a folder under the Templates node.

:::note
You can't create new folder locations.
:::
...
**Step 5 –** Check the Append Import Details to Modified Entries checkbox to append the import information to a pre-existing item instead of overwriting it. This means that if a policy, template, or collection exists in the system with the same name as in the XML file, the system will add the
