...
Use the Collection Method page to select the collection method the data collector uses. This
...
- Copy the log locally and process (Not available for all query scenarios) – Extract events from an
  offline log by moving the log to the Access Analyzer Console and processing it on the local
  host instead of the target host. To use this option, the log type selected for the query
  must be **Windows Event Log (Archived)**.

The Applet Options section is only visible when you select the **Using server side applet**
collection method.
