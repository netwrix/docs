...
Remember, to use Azure login with the windows application,
...
the conventional AD, the Entra ID is pushing them automatically to the server. For this a so-called
[SCIM service](https://en.wikipedia.org/wiki/System_for_Cross-domain_Identity_Management) is used.

To log in to Netwrix Password Secure, after entering the username a popup opens for the
...
- Add an own application that isn't listed in the Azure Gallery – in the example, the application is named
...
- In the toolbar, click "Manage profiles" in the category "Entra ID"
...
Secure Click "Test Connection" When the test has been successful, click "Save" at the top of the
...
**CAUTION:** that Azure establishes the connection to Netwrix Password Secure. For this,
...
- Click "Netwrix Netwrix Password Secure" and navigate to "Authentication"
- Click "Add a platform", select "Web" and configure the required URIs:
...
Click "Add a platform", select "Mobile & desktop applications" and configure the required
mobile-app URI:
...
Navigate to your Netwrix Password Secure App registration -> Certificates & secrets ->
Client secret
...
Finally, the API permissions for the Azure API have to be set, so the log in to can be performed
successfully.
