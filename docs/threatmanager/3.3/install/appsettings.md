...
Each service ships with an `appsettings.default.json` file that contains the default settings that
apply when no override is present. Every upgrade replaces this file, so don't edit it
