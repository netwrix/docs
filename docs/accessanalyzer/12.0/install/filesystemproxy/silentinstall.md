...
Install the File System Proxy Service on the targeted proxy servers with a
silent installer:
...
- To add a non-default install directory, append `PRODUCTDIR="[path]"` to the command.
    ...
    - `path` – The installation directory path, which must include
...
- `secret` – The password for the `DOMAIN\USERNAME` credentials (within quotes)
