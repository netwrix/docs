...
## New passwords and records overview

Saving a record/password stores information in the MSSQL database. This process is started in the
Passwords module for the client. It's accessed either via the icon in the ribbon, using the
keyboard shortcut "CTRL + N" or via the context menu that's accessed using the right mouse button
...
When creating a new record, it's possible to select from all the forms for which the logged-in user
...
Irrespective of the selected form, it's always possible to define the validity and tags for a
...
- The **validity** defines an end date until which the record is valid. This information can be
  evaluated e.g. in the logbook or in reports. It's thus possible to create a list of all expired
...
them have already been described in the Authorization concept section. It's important to note here
...
:::note
The user right Allow sharing of personal passwords lets you define that personal passwords can't be released to other users.
:::
