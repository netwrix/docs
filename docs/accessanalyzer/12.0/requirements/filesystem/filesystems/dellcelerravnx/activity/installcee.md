...
To install Dell CEE, complete the following steps.
...
:::info
Use the latest version of .NET Framework and Dell CEE with the
...
After Dell CEE installation is complete,
[connect the Data Movers to the Dell CEE Server](#connect-data-movers-to-the-dell-cee-server).
...
After you configure the `cepp.config` file, configure and enable monitoring with
