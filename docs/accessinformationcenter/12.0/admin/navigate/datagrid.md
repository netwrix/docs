...
A Search box above a table's header row lets you filter the table data.
...
A filter icon to the right of each column name lets you apply a column-specific
...
The filter icon is highlighted orange for a column with an applied filter. To clear a filter, click the filter icon and click **Clear**.
...
The column selector menu shows all available columns for the table. Checked columns appear in the table.
Unchecked columns don't appear.
