...
**2. Findings sections (only for non-clean checks).** For every row in the overview table whose status is not ✓ Clean, add a short table below with only the findings that need a decision. One section per tool (Vale / Dale / Derek). Do not add sections for tools that are entirely clean.
...
- **Title-change → link-text sweep (rulebook §8).** If a title fix was applied to any file, run a repo-wide search for internal markdown links whose visible text uses the *old* title and update the link text to match the new title. URL resolution alone is not sufficient — visible link text must describe the current target. Concrete: `grep -rE '\[<old title>\]\((pathname://)?/docs/' docs/` — the `pathname://` prefix is optional in the pattern because both link forms are in active use; matching only the bare `/docs/` form would silently skip `pathname:///docs/...` links (adjust for slashes/special chars). Apply the link-text updates as part of the same commit as the title fix.
