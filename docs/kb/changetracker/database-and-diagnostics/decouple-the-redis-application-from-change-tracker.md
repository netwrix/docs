...
knowledge_article_id: ka0Qk000000DoevIAC
---
...
> **NOTE:** The below relates to RHEL. If using CentOS, this line can be dropped.
...
> **NOTE:** Create a complex password using the line below:
...
Configure a Redis Password in the Redis configuration file:
...
Has Database Username/Password authentication been removed from the connection? 

Please provide a yes or no answer (yes).
...
Has the Password requirement for the Cache server (Redis) connection been removed? Please provide a yes or no answer (no).
...
Please provide the Password required to connect to the Cache server (Redis) <YourRedisPassword>.
```

To confirm that the Redis configuration has changed, check the Production JSON file using a text editor. The Redis entries within the editor should match the above settings that you entered.
