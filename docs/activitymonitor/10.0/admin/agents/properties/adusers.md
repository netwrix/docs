...
![AD Users Tab](/images/activitymonitor/9.0/admin/agents/properties/aduserstab.webp)

The configurable options are:

- Domain Controllers (IPs and FQDNs) – IP addresses or FQDN of domain controllers. Enter IP addresses or
  FQDNs as separate addresses with space, comma (,), semicolon (;), or a multi-line
...
- Cache TTL for successful results –Specify the caching interval (time-to-live) for successful AD
  ...
  is cached for the specified time. Use large TTL values because user information doesn't often change.
- Cache TTL for failed results – Specify the caching interval (time-to-live) for failed AD
  ...
  domain controllers and improve performance of event processing. Use short TTL values to make the product report accurate user information.
