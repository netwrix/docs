...
password. The password that is generated follows the complexity rules configured in the related
password policy associated with that platform. The Default policy is used if a password policy
can't be determined. See the [Platforms Page](/docs/privilegesecure/4.2/admin/interface/platforms/overview.md) topic for additional
information.
...
password policy set in Privilege Secure, the managed account can't be created and the session fails
.
...
- Must Start With / Must End With — Lists cases the password complexity policy enforces. This
...
- Length — The maximum number of characters the password complexity policy enforces
- Max Consecutive Chars — The maximum number of consecutively occurring characters that the password
complexity policy allows
- Characters to Exclude — Custom characters the password complexity policy excludes.
