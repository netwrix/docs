...
rule properties page. Complete the following steps to configure the email message options for a policy.
...
Choose values from the days dropdown lists to specify when to send emails. By default, emails
...
and don't include any other text. The Password Policy Enforcer Mailer reads the email body from the specified file.
