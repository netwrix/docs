...
The 1-Box_Activity Scans Job collects the data which will be further analyzed to provide
...
- Activity Scan – Collects activity data which will be further analyzed to provide
  visibility into user activity events within Box.

Before the first execution, authenticate to the targeted Box environment if you haven't already done
so while configuring the 1-Box_Access Scans Job. This is done on the Authenticate page of the
Activity Scan query. Additionally, the following default configurations are
commonly customized:
