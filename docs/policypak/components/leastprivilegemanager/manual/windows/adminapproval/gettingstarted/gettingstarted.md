...
If there’s no PolicyPakLeast Privilege Manager rule to automatically elevate an
application (or allow it to bypass SecureRun™), the user is prompted with a special dialog to
request access.

:::note
See the [Admin Approval demo](/docs/policypak/components/leastprivilegemanager/videolearningcenter/adminapproval/demo.md) video for
PolicyPak Least Privilege Manager Admin Approval mode setup and
in action.
:::


PolicyPak Least Privilege Managerr’s Admin Approval works as if
...
- The PolicyPak Least Privilege Manager policies, which establish
  Admin Approval and its secret key
- Securing the secret key (if using Group Policy method)
- The PolicyPak Least Privilege Manager Admin Approval Tool
...
Admin Approval will show the user the PolicyPak Admin Approval prompt instead of the
...
- The app is marked as requiring elevation by its developer (in the app manifest). You can see these
  ...
  icon.
- The app is a legacy installer. This is defined by both Windows and PolicyPak as a
...
- The app is any installer and PolicyPak ’s Admin Approval **Enforce Admin Approval
  for all installers** option is enabled (explained later).
- PolicyPak SecureRun™ is enabled, and the executable is run by someone not on the
  SecureRun™ list.
- The user right-clicks a file and selects **Run with PolicyPak**.
