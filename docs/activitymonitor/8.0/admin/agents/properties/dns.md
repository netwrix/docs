...
- Enable local DNS cache service – Select this checkbox to enable the local DNS cache service. Leave
  ...
  option if your DNS infrastructure can't handle the load (requests take hundreds of milliseconds)
  during peak hours.
...
- Cache TTL for failed results – Specify the caching interval (time-to-live) for failed DNS
  responses. The default is 1 minute. When a DNS query can't resolve an IP address or host-name,
...
- Refresh throttle time – Specify the time interval between DNS queries that the DNS Cache service
  ...
  If the DNS Cache service is used, the records are automatically updated when the TTL expires. Use this
  option to limit the number of DNS requests the service sends to update the cache. A
  throttling period of 100 milliseconds limits the update task to 10 requests per second.
