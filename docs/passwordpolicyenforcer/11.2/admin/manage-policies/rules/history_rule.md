...
The 100-hash limit allows users to potentially reuse a password sooner than the configured number of days if they cycle through passwords quickly. Use the [Minimum Age rule](minimum_age_rule.md) to stop this. For example, if you configure the History rule to not allow password reuse for 365 days, then set the minimum password age to four or more days. This limits users to a maximum of 91 password changes every 365 days, ensuring that they can't exceed their storage limit.
