...
enables the automatic exchange of certificates between the Enterprise Manager and the Agent. The
method you use to install the Agent determines how you apply the enrollment secret.
...
- If installing the Agent manually, the enrollment secret must be entered in the Certificates window
  ...
  [Manual Agent Deployment](/docs/threatprevention/8.0/install/agent/manual.md) topic for additional information.
...
![Enrollment Secret Configuration window](/images/threatprevention/8.0/admin/agents/window/enrollmentsecret.webp)
...
- When deploying the Agent from the Administration Console, the system automatically creates a new enrollment secret, which invalidates any previous secret that hasn't expired.
