...
The restriction of resource allocations is done from a filter. To do this, create a [Dimension](../../integration-guide/toolkit/xml-configuration/metadata/dimension) to define which EntityTypes the filters will apply to.
...
To be able to filter with the dimensions previously created, first create [Single Role](../../integration-guide/toolkit/xml-configuration/provisioning/singlerole) which will serve as a restriction to the assignment of ResourceTypes for a given source.
...
## Assign the Role Based on the Dimension
 ...
 will define a [Single Role Rule](../../integration-guide/toolkit/xml-configuration/provisioning/singlerolerule) on the "Title"; dimension with a given value to restrict the allocation of a resource in only one case.
...
This part will link a SingleRole to a ResourceType. This implies that the allocation of a target resource to a source will only be done if the SingleRole rules are verified.
...
In the example above the ResourceType doesn't need a TypeRule because the NavigationRule already serves as an allocation context.
