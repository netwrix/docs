...
| [Scan SharePoint Online](./sharepoint-online.md) | **SharePoint Online** | **Client ID/certificate** | Access scan, then Sensitive data scan | Data security dashboard, SharePoint reports |

The guides are independent, but they work best in pairs. File system permission reports show account and group names only after an Active Directory Identity sync has run for the domain, so follow the Active Directory guide alongside the SMB file servers guide. SharePoint permission reports expand group membership using the latest Entra ID Identity sync for the same tenant; without it, Access Analyzer calculates permissions from SharePoint data alone. Pair the SharePoint Online guide with the Entra ID guide.
