...
See the [Application Server Requirements](/docs/threatmanager/3.1/requirements/server.md) topic for information on
...
**Step 3 –** In the Type dropdown, select Credential Profile.
...
Threat Manager updates the Credential Profile name and/or description.

## Edit Credential

To edit a credential within a Credential Profile, for example if the account password needs to be updated:
...
Threat Manager updates the credential within the Credential Profile.
...
credentials to a single profile. During certain actions, these credentials are enumerated
until one can execute the task.
...
- Actions
    ...
    - The $Credential parameter is populated with the most appropriate credential from the list
        ...
        - The domain of the event is matched against the domain of the credential
