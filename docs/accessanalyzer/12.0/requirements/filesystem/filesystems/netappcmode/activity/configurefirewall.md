...
disable the host name validation if the agent can't resolve the FQDN of the LIF.
...
agent's settings. The agent adds this port to Windows Firewall exclusions automatically. Ensure the
port isn't blocked by other firewalls between ONTAP and the agent.
