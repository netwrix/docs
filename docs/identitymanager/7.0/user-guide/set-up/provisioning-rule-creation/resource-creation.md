...
The application of a resource type rule can depend on the assignment of a single role, and/or user dimensions.
...
4. Fill in the fields.
    ...
    ![Create a Resource Type Rule](/images/identitymanager/provrules_typerule_v602.webp)
...
Any modification in a resource type rule is taken into account when launching the role model computation task, in the **Resource Types** frame of the corresponding connector's overview page, via **Jobs** > **Compute Role Model**.
...
If the type rule uses a single role as a criterion, and the user has said role, then both the resource type and the role will be displayed in the user's permissions, **but only if** the role is related to a [Compute a Navigation Property](../../../user-guide/set-up/provisioning-rule-creation/navigation-property-computation). Otherwise, only the resource type will be visible.
