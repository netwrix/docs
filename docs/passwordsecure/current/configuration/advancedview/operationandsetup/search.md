...
With the help of the search, you can find data stored in the database efficiently
...
open. This is a full-text search that scans all fields and tags except the password field.
...
The fast search is closely linked to the [Filter](/docs/passwordsecure/current/configuration/advancedview/operationandsetup/filter/filter.md), because search queries are
converted directly into one or several content filters. You can also separate search terms using
spaces, for example, **Cook Daniel**. This search creates two separate content filters,
...
With the list search in the header of the [List view](/docs/passwordsecure/current/configuration/advancedview/operationandsetup/list_view.md), the results of the
filter can be searched further. This type of search is available in almost every list. Scans only
the filtered results. Password fields aren't searched. The search is live, so the result
