...
The Compilation tab controls where your PolicyPak source files (pXML) are saved and
...
the location where the PolicyPak Application Settings Manager Group Policy Editor will
look for compiled extensions, so it's best to leave this as it is.

![using_designstudio_tools_2](/images/policypak/applicationsettings/designstudio/tools/options/using_designstudio_tools_2.webp)
...
PolicyPak DesignStudio as a standard user (non-administrator) you are prompted for an
...
Additionally, you can see the "Preferred C++ Compiler" in the Compilation tab. PolicyPak Application Settings Manager is actually compatible with two compilers: Microsoft Visual C++
2008 (Express and Full Editions) and MinGW compiler. However, it is recommended that you use the
Microsoft Visual C++ 2008 compiler. Lastly, you can also see the PolicyPak compiler
