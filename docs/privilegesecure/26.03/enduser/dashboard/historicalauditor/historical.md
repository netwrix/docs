---
title: "Historical Dashboard: Auditor"
description: "Historical Dashboard: Auditor"
sidebar_position: 40
---
...
![historical](/images/privilegesecure/26.03/accessmanagement/admin/dashboard/historical.webp)
...
replay of the remote session. See the [Replay Viewer Window](/docs/privilegesecure/26.03/admin/dashboard/historical/replayviewer.md) topic
for additional information.

- Rocket icon — Launches the same session (same activity on the same resource with the same
connection profile) for any historical session that isn't a Credential-based session
- View logs icon — Opens the Session Logs window to view the action log for the selected
session. See the [Session Logs Window](/docs/privilegesecure/26.03/admin/dashboard/historical/sessionlogs.md) topic for additional
information.
...
for additional information.
- Host — Resource that the user run the activity on. Click the link to view additional details.
...
When metadata is searched and a recorded session is opened, the playback jumps to the point of the recording
