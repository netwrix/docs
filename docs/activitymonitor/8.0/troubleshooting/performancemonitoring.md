...
the throughput to 10 events per second). Use the average and maximum values of DNS Queries Time, Resolved SIDs Time, and Translated UIDs Time to estimate the response time.
:::
...
Use the following steps to register the Activity Monitor performance counters on each SAM Agent server.
...
Use the following steps to unregister the Activity Monitor performance counters on each SAM Agent server.
...
After the services restart, the Activity Monitor performance counters are unregistered.
