...
- These notification elements aren't present in the configuration
...
:::note
**For Scaffolded Jobs:**
If your job is generated by `CreateAgentSynchroComplete` or `CreateConnectorSynchroComplete`, manual job definition edits aren't required. Add the notification elements (Step 1) and the scaffolding will automatically generate `SendNotificationsTask`.
:::
...
2. **Verify notification behavior**:
   ...
   - Re-run the job immediately and verify that duplicate notifications aren't sent (ReminderInterval is working)
