...
| IncludeManualAssignmentNotAllowed <span class="optionalAttribute">default value: true</span> | **Type:** Boolean **Description:** `true` to include in the certification the resources that can't be requested manually, i.e. those from [resource types](/docs/identitymanager/current/integration-guide/toolkit/xml-configuration/provisioning/resourcetype#properties) with `ApprovalWorkflowType` set to `ManualAssignmentNotAllowed`. |
