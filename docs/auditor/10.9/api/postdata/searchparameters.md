...
the [/netwrix/api/v1/activity_records/search](/docs/auditor/10.9/api/searchactivityrecords.md) endpoint. The Search
parameters file includes one or more filters with operators and values (e.g., to find entries where
_data source_ is _SharePoint_); it may also contain a [Continuation Mark](/docs/auditor/10.9/api/postdata/continuationmark.md).
