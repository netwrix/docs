...
When activated, the AD Agents filters out the event data according to configuration defined in the
...
interest where as true ‘machine accounts’ isn't. By supplying either an explicit list of gMSA
...
Address, NETBIOS host name, or DNS host name) must be known to effectively exclude
