...
Before configuring the Pubic Folder Action Module Wizard, scope the source data table to ensure
the actions apply only to the folders you want to change.

:::warning
Be careful when using this action module. Apply only the required changes, and only to the target
folders you want to change. Always verify the data before executing any action.
:::


:::info
Although rollbacks for some actions are available, avoid using one when possible
:::
