...
Agent is connected to a legitimate Enterprise Manager. Before launching the Threat Prevention
...
- Configure additional Agent options as desired:
  ...
  - Start Agent Service – This option starts the Threat Prevention Windows Agent service after the
    ...
    **Start Agent Service** checkbox is disabled because Agent installer doesn't obtain a
...
:::note
This page isn't displayed when "Custom-managed" is selected on the CA Certificate
Configuration wizard page .It is also not displayed when the Agent is reinstalled on a machine and
the Certsinfo folder wasn't manually deleted, in which case the original certificates are re-used.
...
Agent. The installer grays out options that aren't applicable to the server where the Agent is
...
- Exchange Server Monitoring – Available as part of the Threat Prevention for Exchange solution.
  - Select this component if the system where the Agent is being installed is running the Exchange
    Server. This option will be grayed-out if this Agent isn't being installed on an Exchange
    Server.
