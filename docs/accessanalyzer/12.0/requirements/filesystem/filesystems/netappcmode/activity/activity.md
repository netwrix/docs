...
depends on the fault tolerance requirements and the expected event flow. As a guideline, the
...
- Select method:
    ...
    - Allow the Activity Monitor to create an FPolicy automatically
        ...
        - FPolicy configuration is automatically updated to reflect the Activity Monitor
          configuration.
        - Requires you to provide a Privileged Access credential.

- Enable the Persistent Store to increase the resilience and control the latency during network
