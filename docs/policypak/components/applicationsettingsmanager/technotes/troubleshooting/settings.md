...
- `ppSwittched.log`: For when Group Policy processes in the background or for when  `GPupdate` is
  run.
- `ppSwitched_ onXmlData.log`: For when directives are delivered via MSI, file or Netwrix PolicyPak Cloud service.
- `ppSwitched_onSchedule.log`: For when directives are re-delivered using the PolicyPak timer mechanism (which is off by default. See the section Automatic Re-Application of
