...
By default, SQL Server stores trace logs in a predefined location (depending on the SQL Server version). For example, SQL Server 2019 error logs are located at
...
must be a separate line. Lines that start with the # sign are treated as comments and are ignored.

**Example:**

`SQLSRV01\MSSQL2016|C:\Logs\1Secure trace logs\`

To change trace logs location for multiple instances of one SQL server, ensure that the UNC paths
