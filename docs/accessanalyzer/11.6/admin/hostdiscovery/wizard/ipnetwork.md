...
- IP Ranges box – Displays the selected range of IP Addresses. Use the links at the top of the box
  to edit the list:
    ...
    - Add as inclusion – Adds information provided in the IP Address Textboxes to the inclusion
      list
    - Add as exclusion – Adds information provided in the IP Address Textboxes to the exclusion
      list
...
only becomes relevant if you also enable Host Inventory Refresh.
:::

If you select **Only include host with the following ports open**, Enterprise Auditor only
...
page, Enterprise Auditor also queries each discovered host for inventory data. This requires the Remote Registry
