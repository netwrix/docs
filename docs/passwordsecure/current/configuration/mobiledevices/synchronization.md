...
First of all, you must note how the synchronization has been configured in the
[Settings](/docs/passwordsecure/current/configuration/mobiledevices/settings_mobileapp.md). A prerequisite for successful synchronization is that
the configured connection is available. This is done via https port 443, which must be enabled on
the server side. After the prerequisites have been met, there are the following triggers for
synchronization:

- A log in to the app takes place
...
The configuration is described in the chapter [Settings](/docs/passwordsecure/current/configuration/mobiledevices/settings_mobileapp.md)
