...
Use the `Export-SIPolicy` and `Import-SIPolicy` commands to export and edit the XML file of an existing policy, and then import (add or replace) the edited policy. See the
...
produced by this command is identical in format to that produced by the Administration Console
import function. See the [Import Window](/docs/threatprevention/8.0/admin/tools/import.md) topic for additional information.
...
Policies exported from the [Policies Interface](/docs/threatprevention/8.0/admin/policies/overview.md) have the option to be
...
:::tip
Remember, the Policy ID and Policy GUID isn't the same as those in the source XML file. Use the
