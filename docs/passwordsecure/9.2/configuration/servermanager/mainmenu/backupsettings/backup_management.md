...
Server Manager. The backups are then generated on the SQL Server.
...
beginning are saved. This saves both time and memory capacity.
...
entered under [Backup settings](/docs/passwordsecure/9.2/configuration/servermanager/mainmenu/backupsettings/backup_settings.md) is used by default.
...
overwritten. The service user lets you create the backup or a service user can be specified
...
is marked in the list in red. The schedule can't be reactivated directly. You need to
...
file are then displayed. Click **Restore** to restore the backup to the existing
database.
