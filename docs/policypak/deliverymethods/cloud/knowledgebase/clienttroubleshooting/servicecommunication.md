...
the Netwrix PolicyPak Cloud client cannot communicate with
PolicyPak Cloud Server.

![256_1_pp-faq-000001](/images/policypak/troubleshooting/cloud/256_1_pp-faq-000001.webp)
...
**Step 1 –** To test if the computer can reach PolicyPak Cloud, use Telnet from the
affected (wait listed) machine and perform a Telnet test to the PolicyPak Cloud
Service.
...
![Telnet Cloud Agent Script](/images/policypak/troubleshooting/cloud/telnetcloudagent.webp)
...
![Communication Passes](/images/policypak/troubleshooting/cloud/communicationpasses.webp)
...
- If the connection fails, that could mean there is some kind of proxy. To configure the proxy for
  the system, see the
  [I always use a proxy and the cloud client cannot seem to make contact with the services (see FAQ Item #3 above first.) What else can I try?](/docs/policypak/deliverymethods/cloud/knowledgebase/clienttroubleshooting/proxyservices.md) topic
...
- If the connection passes, that could mean the Date and Time are wrong on the machine. First
  ...
  [I am getting an error about "GPSVC failed at sign-in". This error occurs exactly one time. What does this mean?](/docs/policypak/installation/knowledgebase/installandupgrade/gpsvcfailed.md) topic
