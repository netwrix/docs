...
Before you create a monitoring plan to audit your VMware hosts, plan for the account that
will collect data – it should meet the following requirements. Contact your
virtual infrastructure administrator if necessary.

**On the target VMware hosts:**

- To collect state-in-time data and audit SSO users, local users, and groups, the account must
...
- If the same account collects both activity and state-in-time data, add it to the
  **Administrators** group for the vCenter SSO domain. That group membership already grants the
  access needed for activity collection, so you don't need to assign a separate **Read-only** role.
