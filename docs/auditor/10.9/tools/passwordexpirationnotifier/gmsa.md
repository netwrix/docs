...
| ------- | ------------ | ---------- |
| `Cannot find an account...` error when specifying the account | You entered the gMSA name without the trailing `$` or with the wrong domain. | Use the `<domain>\<gmsa-name>$` format, for example `CONTOSO\penGmsa$`. |
