...
collection level, 0.Collection Job Group. After these are assigned to the job group, it can be run
...
Assign the host list under the **SharePoint** > **0.Collection** >
...
contains both on-premises and online credentials, the online credentials must be listed
...
option and select the appropriate Connection Profile from the dropdown menu.
...
jobs according to the workflow you want. The other SharePoint Solution sub-job groups can be run in
...
The Access Information Center requires the 2-SPAA_BulkImport Job's default analysis
tasks to run before permission/access reports are accessible. For activity reports, the Access
Information Center requires the default analysis tasks of both the 2-SPAA Bulk Import Job
and the 2-SPAC Bulk Import Job to run.
...
- Stale File defaults to 365 days

    - Customize within the **4.Content** > **SP_StaleFiles** Job

- Stale Teams defaults to 30 days
...
including SharePoint, which incorporates this analyzed data into further analysis.
...
but it depends on the 0.Collection Job Group and the user-modified CSV files.
