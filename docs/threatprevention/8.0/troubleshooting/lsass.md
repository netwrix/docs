...
the first five minutes from last reboot, then the Agent assumes it's due to the above situation and
...
- Second, it sets itself to “manual start” so that the Agent instrumentation isn't loaded on reboot
...
After the counter patch provided by Netwrix is deployed, you can restart the Agent from the
Administration Console. In this case, the console will first check if the Agent is set to
...
:::note
Register a primary and a secondary technical contact for your organization with Netwrix throughout
the lifetime of Threat Prevention usage. Netwrix will issue notifications to these registered
technical contacts related to Microsoft-issued KBs affecting LSASS and the counter patch (if one is
needed). Contact the organization’s Netwrix Sales Representative or
...
conflict with the Agent, Netwrix first sends a notification informing the registered technical
...
or more before Microsoft issues the incompatible KB.
...
Complete the following steps to enable email notifications for the _LSASS process terminated_ alert
under Operations.
...
[Configure SMTP Host Information](/docs/threatprevention/8.0/admin/configuration/systemalerting/email.md#configure-smtp-host-information)
...
**Step 3 –** Create a Message Profile for the LSASS notification with the recipients to be
notified when the LSASS process terminates. See the
[Create Message Profiles](/docs/threatprevention/8.0/admin/configuration/systemalerting/email.md#create-message-profiles)
topic for additional information.

![System Alerting window - LSASS Process Terminated alert](/images/threatprevention/8.0/troubleshooting/lsassprocessterminated.webp)
...
from the dropdown menu. See the [Email Tab](/docs/threatprevention/8.0/admin/configuration/systemalerting/email.md) topic
